...
impl<W: Copy + Store<vec128_storage>, G> Store<vec256_storage> for x2<W, G> {
    ...
    unsafe fn unpack(p: vec256_storage) -> Self {
        let p = p.split128();
        x2::new([unsafe { W::unpack(p[0]) }, unsafe { W::unpack(p[1]) }])
    }
}
...
impl<W: StoreBytes + BSwap + Copy, G> StoreBytes for x2<W, G> {
    ...
    unsafe fn unsafe_read_le(input: &[u8]) -> Self {
        ...
        x2::new([unsafe { W::unsafe_read_le(input.0) }, unsafe { W::unsafe_read_le(input.1) }])
    }
    #[inline(always)]
    unsafe fn unsafe_read_be(input: &[u8]) -> Self {
        let input = input.split_at(input.len() / 2);
        x2::new([unsafe { W::unsafe_read_be(input.0) }, unsafe { W::unsafe_read_be(input.1) }])
    }
    #[inline(always)]
...
impl<W: StoreBytes + BSwap + Copy> StoreBytes for x4<W> {
    ...
    unsafe fn unsafe_read_le(input: &[u8]) -> Self {
        let n = input.len() / 4;
        x4([
            unsafe { W::unsafe_read_le(&input[..n]) },
            unsafe { W::unsafe_read_le(&input[n..n * 2]) },
            unsafe { W::unsafe_read_le(&input[n * 2..n * 3]) },
            unsafe { W::unsafe_read_le(&input[n * 3..]) },
        ])
    }
    #[inline(always)]
    unsafe fn unsafe_read_be(input: &[u8]) -> Self {
        let n = input.len() / 4;
        x4([
            unsafe { W::unsafe_read_be(&input[..n]) },
            unsafe { W::unsafe_read_be(&input[n..n * 2]) },
            unsafe { W::unsafe_read_be(&input[n * 2..n * 3]) },
            unsafe { W::unsafe_read_be(&input[n * 3..]) },
        ])
    }
    #[inline(always)]
    fn write_le(self, out: &mut [u8]) {

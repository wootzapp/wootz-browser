...
impl<E: Engine> PermutationArgument<E> {
    ...
    pub fn open_commitments_to_s_prime(&self, challenges: &Vec<E::Fr>, y: E::Fr, z_prime: E::Fr, srs: &SRS<E>) -> PermutationProof<E> {
        let n = self.non_permuted_coefficients[0].len();
        ...
        for (p, r) in self.non_permuted_coefficients.iter().zip(challenges.iter()) {
        ...
        let f = polynomial_commitment_opening(0, n, Some(v_neg).iter().chain_ext(polynomial.iter()), yz, &srs);

        mut_distribute_consequitive_powers(&mut polynomial[..], y, y);

        let e = polynomial_commitment_opening(0, n, Some(v_neg).iter().chain_ext(polynomial.iter()), z_prime, &srs);

        PermutationProof { v_zy: v, e_opening: e, f_opening: f }
    }
    ...
    pub fn make_argument(
        ...
        let s_zy_opening = polynomial_commitment_opening(0, n, Some(s_zy_neg).iter().chain_ext(s_polynomial.iter()), z, &srs);

        // Sj(P4j)^β (P1j)^γ is equal to the product of the coefficients of Sj′(P3j)^β (P1j)^γ

        let p_1_values = vec![E::Fr::one(); n];
        let p_3_values: Vec<E::Fr> = (1..=n)
            .map(|el| {
                let mut repr = <<E as ScalarEngine>::Fr as PrimeField>::Repr::default();
                repr.as_mut()[0] = el as u64;
                let fe = E::Fr::from_repr(repr).unwrap();
        ...
        for ((non_permuted, inv_permuted), permutation) in self
            .non_permuted_at_y_coefficients
            .into_iter()
            ...
            .zip(self.permutations.into_iter())
        {
            ...
            {
                let p_4_values: Vec<E::Fr> = permutation
                    .into_iter()
                    .map(|el| {
                        let mut repr = <<E as ScalarEngine>::Fr as PrimeField>::Repr::default();
                        repr.as_mut()[0] = el as u64;
...

        {
            ...
            let valid = GrandProductArgument::verify_ab_commitment(n, &randomness, &a_commitments, &b_commitments, &grand_product_openings, y, z, &srs);
            assert!(valid, "ab part of grand product argument must be valid");

            let randomness = (0..3).map(|_| E::Fr::rand(rng)).collect::<Vec<_>>();
            let valid = GrandProductArgument::verify(n, &randomness, &a_zy, &grand_product_challenges, t_commitment, &c_commitments, &proof, y, z, &srs);
    ...
    pub fn verify_s_prime_commitment(
        ...
        let h_alpha_x_term = multiexp(Some(proof.e_opening).iter().chain_ext(Some(proof.f_opening).iter()), randomness.iter()).into_affine();

        let s_r = multiexp(commitments.iter(), challenges.iter()).into_affine();

        let p2_r = multiexp(specialized_srs.p_2.iter(), challenges.iter()).into_affine();

        let h_term = multiexp(Some(s_r).iter().chain_ext(Some(p2_r).iter()), randomness.iter()).into_affine();
    ...
    pub fn verify(s_commitments: &Vec<E::G1Affine>, proof: &PermutationArgumentProof<E>, z: E::Fr, srs: &SRS<E>) -> bool {
        let g = srs.g_positive_x[0];
    ...
    pub fn make_signature(coefficients: Vec<Vec<E::Fr>>, permutations: Vec<Vec<usize>>, y: E::Fr, z: E::Fr, srs: &SRS<E>) -> SignatureOfCorrectComputation<E> {
        ...
        let (proof, grand_product_signature) = {
            let (proof, grand_product_signature) = argument.make_argument_with_transcript(&mut transcript, y, z, &srs);
    ...
    // Argument a permutation argument. Current implementation consumes, cause extra arguments are required
    pub fn make_argument_with_transcript(self, transcript: &mut Transcript, y: E::Fr, z: E::Fr, srs: &SRS<E>) -> (PermutationArgumentProof<E>, GrandProductSignature<E>) {
        ...
        for ((((non_permuted, inv_permuted), permutation), beta), gamma) in self
            .non_permuted_at_y_coefficients
            .into_iter()
            .zip(self.inverse_permuted_at_y_coefficients.into_iter())
            .zip(self.permutations.into_iter())
        ...
        let grand_product_signature = GrandProductArgument::create_signature(transcript, grand_products, y, z, &srs);

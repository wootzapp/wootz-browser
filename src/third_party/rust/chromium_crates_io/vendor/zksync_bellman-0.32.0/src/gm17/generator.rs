...
impl<E: Engine> ConstraintSystem<E> for KeypairAssembly<E> {
        ...
        LC: FnOnce(LinearCombination<E>) -> LinearCombination<E>,
    {
        ...
        fn quadruple<E: Engine>(coeff: E::Fr) -> E::Fr {
            let mut tmp = coeff;
            tmp.double();
            tmp.double();
        ...
        fn eval<E: Engine>(l: LinearCombination<E>, inputs: &mut [Vec<(E::Fr, usize)>], aux: &mut [Vec<(E::Fr, usize)>], this_constraint: usize) {
        ...
        let y = self.alloc(|| format!("SAP reduction y_{}", i), || Ok(E::Fr::one())).expect("must allocate SAP reduction variable");
        self.num_r1cs_aux -= 1;
    ...
    C: Circuit<E>,
{
    ...
    // so input 0 (identity) is constrained as 1*1=1
    {
        ...
        fn eval_lc<E: Engine>(l: LinearCombination<E>, inputs: &mut [Vec<(E::Fr, usize)>], aux: &mut [Vec<(E::Fr, usize)>], this_constraint: usize) {
            for (index, coeff) in l.0 {
                match index {
                    Variable(Index::Input(id)) => inputs[id].push((coeff, this_constraint)),
                    Variable(Index::Aux(id)) => aux[id].push((coeff, this_constraint)),
    ...
    for i in 1..assembly.num_inputs {
        assembly.enforce(|| "", |lc| lc + Variable(Index::Input(i)), |lc| lc + Variable(Index::Input(0)), |lc| lc + Variable(Index::Input(i)));
    }

    // check that each input generates 2 constraints
    assert_eq!(
        num_inputs_without_identity * 2 + num_constraints_before_inputs_constraining,
        assembly.num_constraints,
        "each input must produce two extra constraints"
    );
    // and that it creates one extra variable
    assert_eq!(
        num_inputs_without_identity + num_aux_before_inputs_constraining,
        assembly.num_aux,
    ...
    if verbose {
        eprintln!("Constraint system size is {}", assembly.num_constraints)
    };
    ...
    let g1_wnaf = g1_wnaf.base(g1, {
        2 * (assembly.num_inputs + assembly.num_r1cs_constraints + assembly.num_r1cs_aux)
            + assembly.num_r1cs_constraints
            + assembly.num_r1cs_aux
    ...
    if verbose {
        eprintln!("Domain length is {} ", domain_length)
    };
...

    {
        // Compute powers of tau
        if verbose {
            eprintln!("computing powers of tau...")
        };
        ...
        if verbose {
            eprintln!("powers of tau stage 1 done in {} s", start.elapsed().as_millis() as f64 / 1000.0);
        };
        ...
        if verbose {
            eprintln!("computing the `G1^(gamma^2 * Z(t) * t^i)` query with multiple threads...")
        };
        ...
        if verbose {
            eprintln!("computing the `G1^(gamma^2 * Z(t) * t^i)` query done in {} s", start.elapsed().as_millis() as f64 / 1000.0);
        };
    }
    ...
    if verbose {
        eprintln!("using inverse FFT to convert to intepolation coefficients...")
    };
    ...
    if verbose {
        eprintln!("powers of tau evaluation in radix2 domain in {} s", start.elapsed().as_millis() as f64 / 1000.0)
    };

    if verbose {
        eprintln!("evaluating polynomials...")
    };
    let start = std::time::Instant::now();
    ...
    fn eval_stage_1<E: Engine>(
        ...
        worker.scope(a_g1.len(), |scope, chunk| {
            for (((((a_g1, a_g2), c_1_g1), c_2_g1), at), ct) in a_g1
                .chunks_mut(chunk)
                ...
                .zip(ct.chunks(chunk))
            {
                ...
                scope.spawn(move |_| {
                    for (((((a_g1, a_g2), c_1_g1), c_2_g1), at), ct) in a_g1.iter_mut().zip(a_g2.iter_mut()).zip(c_1_g1.iter_mut()).zip(c_2_g1.iter_mut()).zip(at.iter()).zip(ct.iter()) {
    ...
    if verbose {
        eprintln!("evaluating polynomials done in {} s", start.elapsed().as_millis() as f64 / 1000.0);
    };

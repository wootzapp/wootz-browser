    ...
    E::Fr: PartialTwoBitReductionField,
{
    pub(crate) fn commit_single_poly<CP: CTPrecomputations<E::Fr>>(
        ...
        let lde = poly
            .clone()
            .bitreversed_lde_using_bitreversed_ntt_with_partial_reduction(worker, params.lde_factor, omegas_bitreversed, &E::Fr::multiplicative_generator())?;
        ...
        Ok(SinglePolyCommitmentData::<E::Fr, _> { poly: lde, oracle: oracle })
    }
    ...
    fn prove_with_setup_precomputed<
        CP: CTPrecomputations<E::Fr>,
        CPI: CTPrecomputations<E::Fr>,
        FP: FriPrecomputations<E::Fr>,
        T: Transcript<E::Fr, Input = <FriSpecificBlake2sTree<E::Fr> as IopInstance<E::Fr>>::Commitment>,
    >(
        self,
        ...
        let a_poly = w_l
            ...
            .ifft_using_bitreversed_ntt_with_partial_reduction(&worker, omegas_inv_bitreversed, &E::Fr::one())?;
        let b_poly = w_r
            .clone_padded_to_domain()?
            .ifft_using_bitreversed_ntt_with_partial_reduction(&worker, omegas_inv_bitreversed, &E::Fr::one())?;
        let c_poly = w_o
        ...
        let z_1_shifted_coset_lde_bitreversed = z_1_shifted
            .clone()
            .bitreversed_lde_using_bitreversed_ntt_with_partial_reduction(&worker, 4, omegas_bitreversed, &E::Fr::multiplicative_generator())?;
...

        {
            ...
            let l_coset_lde_bitreversed = l_0
                .clone()
                .bitreversed_lde_using_bitreversed_ntt_with_partial_reduction(&worker, 4, omegas_bitreversed, &E::Fr::multiplicative_generator())?;
        ...
        let witness_opening_request_at_z = WitnessOpeningRequest {
            ...
            opening_point: z,
            opening_values: vec![a_at_z, b_at_z, c_at_z, z_1_at_z, z_2_at_z, t_low_at_z, t_mid_at_z, t_high_at_z],
        };

        let witness_opening_request_at_z_omega = WitnessOpeningRequest {
            polynomials: vec![&z_1_commitment_data.poly, &z_2_commitment_data.poly],
        ...
        let setup_opening_request = SetupOpeningRequest {
            ...
            opening_point: z,
            opening_values: vec![q_l_at_z, q_r_at_z, q_o_at_z, q_m_at_z, q_c_at_z, s_id_at_z, sigma_1_at_z, sigma_2_at_z, sigma_3_at_z],
        };

        let _ = Self::multiopening(
            vec![witness_opening_request_at_z, witness_opening_request_at_z_omega],
...
mod test {
    ...
    fn test_bench_redshift() {
        ...
        for (size, coset_schedule) in sizes.into_iter().zip(coset_schedules.into_iter()) {
            ...
            let (_, setup_precomp) = setup_with_precomputations::<Transparent252, _, _, Transcr>(&circuit, &params, &omegas_bitreversed).unwrap();
            ...
            let _ = prover
                .prove_with_setup_precomputed::<_, _, _, Transcr>(&setup_precomp, &params, &worker, &omegas_bitreversed, &omegas_inv_bitreversed, &omegas_inv_bitreversed_for_fri)
                .unwrap();

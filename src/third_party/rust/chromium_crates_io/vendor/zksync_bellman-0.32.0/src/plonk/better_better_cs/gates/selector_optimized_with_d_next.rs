...
impl<E: Engine> MainGate<E> for SelectorOptimizedWidth4MainGateWithDNext {
    ...
    fn format_term(mut instance: MainGateTerm<E>, padding: Variable) -> Result<(SmallVec<[Variable; DEFAULT_SMALLVEC_CAPACITY]>, SmallVec<[E::Fr; DEFAULT_SMALLVEC_CAPACITY]>), SynthesisError> {
        assert!(
            instance.num_multiplicative_terms <= 1,
    ...
    fn contribute_into_quotient_for_public_inputs<'a, 'b>(
        ...
        // Q_D_next * D_next
        let q_d_next_ref = get_from_map_unchecked(PolynomialInConstraint::from_id(PolyIdentifier::GateSetupPolynomial(name, 7)), ldes_storage);
    ...
    fn contribute_into_linearization_commitment_for_public_inputs<'a>(
        ...
        let commitment = commitments_storage
            .get(&PolyIdentifier::GateSetupPolynomial(name, Self::AB_MULTIPLICATION_TERM_COEFF_INDEX))
            .ok_or(SynthesisError::AssignmentMissing)?;
        ...
        // Q_const
        let commitment = commitments_storage
            .get(&PolyIdentifier::GateSetupPolynomial(name, Self::CONSTANT_TERM_COEFF_INDEX))
            .ok_or(SynthesisError::AssignmentMissing)?;

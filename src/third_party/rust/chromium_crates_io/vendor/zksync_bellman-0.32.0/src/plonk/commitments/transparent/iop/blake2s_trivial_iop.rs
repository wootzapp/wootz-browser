...
lazy_static! {
    static ref BASE_BLAKE2S_PARAMS: State = { Params::new().hash_length(32).key(b"Squeamish Ossifrage").personal(b"Shaftoe").to_state() };
}
...
impl<F: PrimeField> Blake2sTreeHasher<F> {
    pub fn new() -> Self {
        Self { encoder: Blake2sLeafEncoder::new() }
    }
}

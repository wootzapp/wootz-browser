...
impl<F: PrimeField, FRI: FriIop<F>, T: Transcript<F, Input = <<<<FRI as FriIop<F>>::IopType as IOP<F>>::Tree as IopTree<F>>::TreeHasher as IopTreeHasher<F>>::HashOutput>> std::fmt::Debug
    for StatelessTransparentCommitter<F, FRI, T>
{
...
impl<F: PrimeField, FRI: FriIop<F>, T: Transcript<F, Input = <<<<FRI as FriIop<F>>::IopType as IOP<F>>::Tree as IopTree<F>>::TreeHasher as IopTreeHasher<F>>::HashOutput>> CommitmentScheme<F>
    for StatelessTransparentCommitter<F, FRI, T>
{
    ...
    fn precompute(&self, poly: &Polynomial<F, Coefficients>) -> Option<Self::IntermediateData> {
        ...
        let original_poly_lde = poly
            .clone()
            .lde_using_bitreversed_ntt(&self.worker, self.lde_factor, &self.precomputed_bitreversed_omegas)
    ...
    fn commit_single(&self, poly: &Polynomial<F, Coefficients>) -> (Self::Commitment, Option<Self::IntermediateData>) {
        ...
        let mut original_poly_lde = poly
            .clone()
            .bitreversed_lde_using_bitreversed_ntt(&self.worker, self.lde_factor, &self.precomputed_bitreversed_omegas, &F::one())
            .expect("must make an LDE");
    ...
    fn open_single(&self, poly: &Polynomial<F, Coefficients>, at_point: F, _opening_value: F, data: &Option<&Self::IntermediateData>, prng: &mut Self::Prng) -> Self::OpeningProof {
        ...
        let precomputations = if data.is_some() { None } else { self.precompute(&poly) };
    ...
    fn open_multiple(
        ...
        assert!(
            f64::from(max_degree as u32) / f64::from(min_degree as u32) < 2.0,
            "polynomials should not have too large degree difference"
        );

        let mut division_results = vec![vec![]; polynomials.len()];

        self.worker.scope(polynomials.len(), |scope, chunk| {
            for ((p, q), at) in polynomials.chunks(chunk).zip(division_results.chunks_mut(chunk)).zip(at_points.chunks(chunk)) {
                scope.spawn(move |_| {
    ...
    fn verify_single(&self, commitment: &Self::Commitment, at_point: F, claimed_value: F, proof: &Self::OpeningProof, prng: &mut Self::Prng) -> bool {
        ...
        for (domain_idx, original_poly_query) in domain_indexes.clone().into_iter().zip(original_poly_queries.iter()) {
        ...
        let valid = FRI::verify_proof_with_challenges(q_poly_fri_proof, domain_indexes, &simulated_q_poly_values, &fri_challenges, &self.fri_params).expect("fri verification should work");
    ...
    fn verify_multiple_openings(
        ...
        for subpoly_index in 0..original_poly_queries_vec.len() {
            ...
            for (domain_idx, original_poly_query) in domain_indexes.clone().into_iter().zip(queries_to_the_same_poly.iter()) {
                let x = lde_domain.generator.pow(&[domain_idx as u64]);

...
bool DomainBlockChecker::IsDomainBlocked(const std::string& host, PrefService* prefs) {
  ...
  if (blocked_list.size() != cached_size_ || current_hash != cached_hash_ || blocked_domains_.empty()) {
...
bool DomainBlockChecker::IsValidDomain(const std::string& domain) {
  ...
  if (HasSuspiciousCharacters(domain)) return false;
  ...
  for (char c : domain) {
    if (c == '.') {
      if (prev_was_dot) return false; // consecutive dots
      prev_was_dot = true;
...
bool DomainBlockChecker::IsValidDomainStructure(const std::string& domain) {
  ...
  for (char c : domain) {
    if (c == '.') {
      ...
      if (current_label_length == 0 || current_label_length > 63) return false;
      current_label_length = 0;
      dot_count++;
      
      // Prevent excessive subdomain depth (security limit)
      if (dot_count > 10) return false;
    } else {
...
bool DomainBlockChecker::IsReservedDomain(const std::string& domain) {
  ...
  static const base::NoDestructor<std::unordered_set<std::string>> reserved_domains({
    "localhost", "local", "test", "invalid", "example", "onion",
    ".localhost", ".local", ".test", ".invalid", ".example", ".onion"
  });
  
  // Check exact match
  if (reserved_domains->count(domain)) return true;
  
  // Check if ends with reserved TLD
  for (const auto& reserved : {".localhost", ".local", ".test", ".invalid", ".example", ".onion"}) {
...
bool DomainBlockChecker::HasSuspiciousCharacters(const std::string& domain) {
  ...
  for (char c : domain) {
    ...
    if (std::isdigit(c)) {
      consecutive_numbers++;
      max_consecutive_numbers = std::max(max_consecutive_numbers, consecutive_numbers);
    } else {
      all_numeric = false;
      consecutive_numbers = 0;
      if (std::isalpha(c)) {
  ...
  // Flag all-numeric domains (except IP addresses, but those should be handled elsewhere)
  if (all_numeric && has_letter == false) return true;
  
  // Flag domains with excessive consecutive numbers (possible IP disguising)
  if (max_consecutive_numbers > 8) return true;
...
size_t DomainBlockChecker::CalculateListHash(const base::Value::List& blocked_list) {
  size_t hash = 0;
  for (const auto& domain_value : blocked_list) {
    if (domain_value.is_string()) {
      const std::string& domain = domain_value.GetString();
      if (!domain.empty()) {
        ...
        hash ^= std::hash<std::string>{}(domain) + 0x9e3779b9 + (hash << 6) + (hash >> 2);
      }
    }
  }
  return hash;

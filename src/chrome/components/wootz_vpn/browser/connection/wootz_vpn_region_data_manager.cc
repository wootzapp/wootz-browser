#include "wootz/components/wootz_vpn/browser/connection/wootz_vpn_region_data_manager.h"
...
#include "wootz/components/wootz_vpn/browser/api/wootz_vpn_api_helper.h"
...
mojom::Region GetRegionFromValue(const base::Value::Dict& value) {
  mojom::Region region;
  if (auto* continent = value.FindString(wootz_vpn::kRegionContinentKey)) {
    region.continent = *continent;
  }
  if (auto* name = value.FindString(wootz_vpn::kRegionNameKey)) {
    region.name = *name;
  }
  if (auto* name_pretty = value.FindString(wootz_vpn::kRegionNamePrettyKey)) {
    region.name_pretty = *name_pretty;
  }
  if (auto* country_iso_code =
          value.FindString(wootz_vpn::kRegionCountryIsoCodeKey)) {
...
WootzVPNRegionDataManager::WootzVPNRegionDataManager(
...
WootzVPNRegionDataManager::~WootzVPNRegionDataManager() = default;

const std::vector<mojom::Region>& WootzVPNRegionDataManager::GetRegions()
    const {
  return regions_;
}

bool WootzVPNRegionDataManager::IsRegionDataReady() const {
  return !regions_.empty();
}

void WootzVPNRegionDataManager::SetSelectedRegion(const std::string& name) {
  local_prefs_->SetString(prefs::kWootzVPNSelectedRegion, name);
...
std::string WootzVPNRegionDataManager::GetSelectedRegion() const {
  DCHECK(!regions_.empty())
      << "regions data must be prepared before panel asks.";

  auto region_name = local_prefs_->GetString(prefs::kWootzVPNSelectedRegion);
...
std::string WootzVPNRegionDataManager::GetDeviceRegion() const {
  return local_prefs_->GetString(prefs::kWootzVPNDeviceRegion);
}

void WootzVPNRegionDataManager::SetDeviceRegion(const std::string& name) {
  local_prefs_->SetString(prefs::kWootzVPNDeviceRegion, name);
}

void WootzVPNRegionDataManager::SetFallbackDeviceRegion() {
  // Set first item in the region list as a |device_region_| as a fallback.
  DCHECK(!regions_.empty());
  SetDeviceRegion(regions_[0].name);
}

void WootzVPNRegionDataManager::SetDeviceRegionWithTimezone(
  ...
  for (const auto& timezones : timezones_value) {
    ...
    for (const auto& timezone : *timezone_list_value) {
      ...
      if (current_time_zone == timezone.GetString()) {
        ...
        if (local_prefs_->GetString(prefs::kWootzVPNSelectedRegion).empty()) {
...
void WootzVPNRegionDataManager::LoadCachedRegionData() {
  ...
  auto* preference = local_prefs_->FindPreference(prefs::kWootzVPNRegionList);
  DCHECK(preference);
...
bool WootzVPNRegionDataManager::NeedToUpdateRegionData() const {
  ...
  const auto last_fetched_date =
      local_prefs_->GetTime(prefs::kWootzVPNRegionListFetchedDate);
...
void WootzVPNRegionDataManager::NotifyRegionDataReady() const {
  if (region_data_ready_callback_) {
    region_data_ready_callback_.Run(!regions_.empty());
  }
}

void WootzVPNRegionDataManager::FetchRegionDataIfNeeded() {
  if (api_request_) {
  ...
  api_request_->GetAllServerRegions(base::BindOnce(
      &WootzVPNRegionDataManager::OnFetchRegionList, base::Unretained(this)));
}

void WootzVPNRegionDataManager::OnFetchRegionList(
  ...
  if (value && value->is_list() &&
    ...
    api_request_->GetTimezonesForRegions(base::BindOnce(
        &WootzVPNRegionDataManager::OnFetchTimezones, base::Unretained(this)));
    return;
  }
...
bool WootzVPNRegionDataManager::ParseAndCacheRegionList(
...
void WootzVPNRegionDataManager::OnFetchTimezones(
...
void WootzVPNRegionDataManager::SetRegionListToPrefs() {
  DCHECK(!regions_.empty());
  ...
  local_prefs_->Set(prefs::kWootzVPNRegionList,
                    base::Value(std::move(regions_list)));
  local_prefs_->SetTime(prefs::kWootzVPNRegionListFetchedDate,
                        base::Time::Now());
}

std::string WootzVPNRegionDataManager::GetCurrentTimeZone() {

...
#endif
{
 public:
  ...
  SecurityPolicyChecker* policy_checker() const { return policy_checker_.get(); }
 ...
 private:
  ...
  void InitializeOTelExporter();
  
  // Called when activity tracking prefs change (endpoint or token)
  void OnActivityTrackingPrefsChanged();
  ...
  std::unique_ptr<SecurityPolicyChecker> policy_checker_;

...
TEST(LacrosUrlHandlingTest, IsURLAcceptedByAsh) {
  ...
  params->accepted_internal_ash_urls = std::vector<GURL>{
      ...
      GURL("wootzapp://version"), GURL("wootzapp://settings")};
  ...
  EXPECT_TRUE(
      lacros_url_handling::IsUrlAcceptedByAsh(GURL("wootzapp://version")));
  EXPECT_FALSE(
      lacros_url_handling::IsUrlAcceptedByAsh(GURL("wootzapp://versions")));
  ...
  EXPECT_FALSE(
      lacros_url_handling::IsUrlAcceptedByAsh(GURL("wootzapp://flags2")));
}

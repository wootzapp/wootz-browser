...
#if BUILDFLAG(IS_ANDROID)
  // WOOTZ: Check if mTLS proxy is enabled
  net::MtlsProxyConfig* mtls_config = net::MtlsProxyConfig::GetInstance();
  if (mtls_config && mtls_config->IsEnabled() && 
      mtls_config->HasClientCertificate()) {
    LOG(INFO) << "Using WootzMtlsProxyConfigService for all traffic";
    base_service = std::make_unique<net::WootzMtlsProxyConfigService>();
  } else {
    // Fall back to system proxy config if mTLS proxy is not enabled

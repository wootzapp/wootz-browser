...
export async function getCognitoRecoveryPasswordV1(client) {
    const idTokenResponse = await authFetchEmbeddedWalletUser(client, ROUTE_AUTH_COGNITO_ID_TOKEN_V1, {
        method: "GET",
    });
...
export async function getCognitoRecoveryPasswordV2(client) {
    const idTokenResponse = await authFetchEmbeddedWalletUser(client, ROUTE_AUTH_COGNITO_ID_TOKEN_V2, {
        method: "GET",
    });
    if (!idTokenResponse.ok) {
        throw new Error(`Failed to fetch id token from Cognito: ${JSON.stringify(await idTokenResponse.json(), null, 2)}`);

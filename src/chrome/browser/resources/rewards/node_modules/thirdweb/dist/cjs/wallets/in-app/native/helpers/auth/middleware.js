...
async function getRecoveryCode(storedToken, client, recoveryCode) {
    if (storedToken.authDetails.recoveryShareManagement ===
        types_js_1.RecoveryShareManagement.CLOUD_MANAGED) {
        if (storedToken.authProvider === types_js_1.AuthProvider.CUSTOM_JWT ||
            storedToken.authProvider === types_js_1.AuthProvider.CUSTOM_AUTH_ENDPOINT) {
            if (!recoveryCode) {
        ...
        types_js_1.RecoveryShareManagement.USER_MANAGED) {
        if (recoveryCode) {
            return recoveryCode;
        }
        throw new Error(errors_js_1.ErrorMessages.missingRecoveryCode);

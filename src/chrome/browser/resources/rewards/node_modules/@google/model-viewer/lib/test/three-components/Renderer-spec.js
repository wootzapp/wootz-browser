...
import { $intersectionObserver, $isElementInViewport, $onResize, $renderer, $scene, $updateSize } from '../../model-viewer-base.js';
    ...
    suite('with two loaded scenes', () => {
        ...
        test('uses the proper canvas when unregistering scenes', () => {
            ...
            expect(renderer.canvas3D.parentElement)
                .to.be.eq(otherScene.canvas.parentElement, 'webgl canvas should be shown with single scene.');
            expect(otherScene.canvas.classList.contains('show'))
        ...
        test('When registered, the scene canvas dimensions match the renderer size', () => {
            renderer.render(performance.now());

...
import { $controls, $promptAnimatedContainer, $promptElement, cameraOrbitIntrinsics, DEFAULT_FOV_DEG, DEFAULT_MIN_FOV_DEG, INTERACTION_PROMPT } from '../../features/controls.js';
import { $scene, $statusElement, $userInputElement } from '../../model-viewer-base.js';
...
suite('Controls', () => {
    ...
    suite('camera-controls', () => {
        ...
        suite('interaction-prompt', () => {
            ...
            suite('when configured to be basic', () => {
                ...
                test('does not have a css animation', () => {
                    const computedStyle = getComputedStyle(element[$promptElement]);
                    expect(computedStyle.animationName).to.be.equal('none');
                });
                test('becomes visible', async () => {
                    await until(() => element[$promptElement].classList.contains('visible'));
                    expect(element[$promptAnimatedContainer].style.opacity)
                        .to.be.equal('1');
                });
            });
        });
        suite('synthetic interaction', () => {
            ...
            test('return one finger to starting point returns camera to starting point', async () => {
                const orbit = element.getCameraOrbit();
                element.interactionPrompt = 'none';
            ...
            test('return two fingers to starting point returns target to starting point', async () => {
                const target = element.getCameraTarget();
            ...
            test('disconnecting the mv from DOM cancels the interaction.', async () => {
                let stopped = false;
            ...
            test('camera-orbit cancels synthetic interaction', async () => {
                ...
                await rafPasses();
                await rafPasses();
                const canceled = waitForEvent(element, 'interact-stopped', (event) => event.detail.source === ChangeSource.NONE);
            ...
            test('user interaction cancels synthetic interaction', async () => {
                ...
                await rafPasses();
                await rafPasses();
                const canceled = waitForEvent(element, 'interact-stopped', (event) => event.detail.source === ChangeSource.USER_INTERACTION);
                dispatchSyntheticEvent(element[$userInputElement], 'keydown', { key: 'PageDown' });
                await canceled;
            });

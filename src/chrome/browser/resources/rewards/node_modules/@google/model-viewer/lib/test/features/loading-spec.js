    ...
    suite('with a second element outside the viewport', () => {
        ...
        suite('scroll to second element', () => {
            ...
            test('first element is not visible', async () => {
                await waitForEvent(element, 'model-visibility', event => event.detail.visible === false);
            });
            test('second element is visible', async () => {
                await waitForEvent(element2, 'model-visibility', event => event.detail.visible === true);
            });
        });
    });
            ...
            suite('auto', () => {
                test('hides poster when element loads', async () => {
                    ...
                    expect(pickShadowDescendant(element))
                        .to.be.not.equal(input, 'the poster should be shown until the model loads');

...
ExtensionFunction::ResponseAction WootzGetBlockedDomainsFunction::Run() {
  Profile* profile = Profile::FromBrowserContext(browser_context());
  if (!profile)
    return RespondNow(Error("No profile"));

  const base::Value::List& list = profile->GetPrefs()->GetList(blocked_domains::prefs::kBlockedDomains);

...
content::WebContents* WebContentsIdToJavaWebContents(int webContentsId) {
  ...
  base::android::ScopedJavaLocalRef<jobject> receiver_from_native =
      Java_WootzAppBackgroundContentService_getBackgroundWebContents(
          env, webContentsId);
  ...
  content::WebContents* web_contents =
      content::WebContents::FromJavaWebContents(receiver_from_native);
  if (web_contents) {
    // Ensure AutomationControllerFactory exists for this WebContents
    if (!automation::AutomationControllerFactory::FromWebContents(
            web_contents)) {
      automation::AutomationControllerFactory::CreateForWebContents(
          web_contents);
      LOG(INFO)
          << "Created AutomationControllerFactory for existing WebContents ID: "
          << webContentsId;
    }
  }
...
void OpenExtensionsById(const std::string& extensionId) {
  ...
  Java_OpenExtensionsById_openExtensionByIdNative(
      env, base::android::ConvertUTF8ToJavaString(env, extensionId));
}

void closeExtensionBottomSheet() {
  JNIEnv* env = base::android::AttachCurrentThread();
  Java_OpenExtensionsById_closeExtensionBottomSheetNative(env);
}
...
ExtensionId GetWootzWalletExtensionId(content::BrowserContext* context) {
  ...
  LOG(ERROR)
      << "JANGID: No matching extension found. Total extensions checked: "
      << count;
...
WootzAPI::WootzAPI(content::BrowserContext* context)
    : browser_context_(context), observer_receiver_(this) {
...
void WootzAPI::StartObserving() {
  ...
  if (!service) {
    LOG(ERROR)
        << "jangid_observer: Failed to get WootzWalletService for profile";
    return;
  }
...
void WootzAPI::OnNewUnapprovedTx(
  ...
  LOG(ERROR) << "jangid_sign: Wootz wallet extension ID: "
  ...
  tx_details.Set("origin",
                 tx_info->origin_info ? tx_info->origin_info->origin_spec : "");

  // Solana transaction data
  if (tx_info->from_account_id->coin == wootz_wallet::mojom::CoinType::SOL) {
    LOG(ERROR)
        << "jangid_sign: Processing Solana transaction data of coin type: "
        << static_cast<int>(tx_info->from_account_id->coin);
      ...
      if (instructions[i]->decoded_data) {
        ...
        for (size_t j = 0; j < decoded_data->params.size(); j++) {
          ...
          if (param->name == "lamports") {
            ...
            tx_details.Set("transaction_amount",
                           base::NumberToString(std::stod(param->value)));
          }
    ...
    if (solana_tx_data->fee_estimation) {
      fee_dict.Set("baseFee", static_cast<double>(
                                  solana_tx_data->fee_estimation->base_fee));
      fee_dict.Set(
          "computeUnits",
          static_cast<double>(solana_tx_data->fee_estimation->compute_units));
      fee_dict.Set("feePerComputeUnit",
                   static_cast<double>(
                       solana_tx_data->fee_estimation->fee_per_compute_unit));
      ...
      LOG(ERROR) << "jangid_sign: Solana fee estimation: base_fee="
                 << solana_tx_data->fee_estimation->base_fee
                 << ", compute_units="
                 << solana_tx_data->fee_estimation->compute_units
                 << ", fee_per_compute_unit="
                 << solana_tx_data->fee_estimation->fee_per_compute_unit;
    }

    tx_details.Set("feeEstimation", std::move(fee_list));
    LOG(ERROR) << "jangid_sign: Solana fee estimation: "
               << tx_details.Find("feeEstimation");
  }
  // Ethereum transaction data
  else if (tx_info->from_account_id->coin ==
           wootz_wallet::mojom::CoinType::ETH) {
    LOG(ERROR)
        << "jangid_sign: Processing Ethereum transaction data of coin type: "
        << static_cast<int>(tx_info->from_account_id->coin);
    ...
    if (tx_info->tx_data_union->is_eth_tx_data()) {
      ...
      LOG(ERROR) << "jangid_sign: Ethereum transaction amount: "
                 << eth_tx_data->value;
      ...
      LOG(ERROR) << "jangid_sign: Ethereum transaction gas price: "
                 << eth_tx_data->gas_price;
      fee_dict.Set("baseFee", std::stod(eth_tx_data->gas_price));

      LOG(ERROR) << "jangid_sign: Ethereum transaction gas limit: "
                 << eth_tx_data->gas_limit;
      ...
      LOG(ERROR) << "jangid_sign: Ethereum fee estimation: "
                 << tx_details.Find("feeEstimation");
    }
    // Ethereum transaction data 1559
    else if (tx_info->tx_data_union->is_eth_tx_data_1559()) {
      const auto& eth_tx_data_1559 =
          tx_info->tx_data_union->get_eth_tx_data_1559();

      LOG(ERROR) << "jangid_sign: Ethereum transaction amount: "
                 << eth_tx_data_1559->base_data->value;
      ...
      LOG(ERROR) << "jangid_sign: Ethereum transaction gas price: "
                 << eth_tx_data_1559->base_data->gas_price;
      fee_dict.Set("baseFee",
                   std::stod(eth_tx_data_1559->base_data->gas_price));

      LOG(ERROR) << "jangid_sign: Ethereum transaction gas limit: "
                 << eth_tx_data_1559->base_data->gas_limit;
      fee_dict.Set("computeUnits",
                   std::stod(eth_tx_data_1559->base_data->gas_limit));
  ...
  std::unique_ptr<Event> event = std::make_unique<Event>(
      events::WOOTZ_ON_NEW_UNAPPROVED_TX, "wootz.OnNewUnapprovedTxAPI",
      std::move(event_args), profile, std::nullopt, GURL(),
      EventRouter::USER_GESTURE_UNKNOWN, mojom::EventFilteringInfo::New());

  LOG(ERROR) << "jangid_sign: Dispatching event to extension: "
             << wootz_wallet_extension_id;
...
void WootzAPI::OnTransactionStatusChanged(
  ...
  LOG(ERROR) << "jangid_sign: Current status: "
             << static_cast<int>(tx_info->tx_status);
  ...
  if (tx_info->tx_status ==
      wootz_wallet::mojom::TransactionStatus::Unapproved) {
    status_str = "unapproved";
  } else if (tx_info->tx_status ==
             wootz_wallet::mojom::TransactionStatus::Approved) {
    status_str = "approved";
  } else if (tx_info->tx_status ==
             wootz_wallet::mojom::TransactionStatus::Rejected) {
    status_str = "rejected";
  } else if (tx_info->tx_status ==
             wootz_wallet::mojom::TransactionStatus::Submitted) {
    status_str = "submitted";
  } else if (tx_info->tx_status ==
             wootz_wallet::mojom::TransactionStatus::Confirmed) {
    status_str = "confirmed";
  } else if (tx_info->tx_status ==
             wootz_wallet::mojom::TransactionStatus::Error) {
    status_str = "error";
  } else if (tx_info->tx_status ==
             wootz_wallet::mojom::TransactionStatus::Dropped) {
    status_str = "dropped";
  } else if (tx_info->tx_status ==
             wootz_wallet::mojom::TransactionStatus::Signed) {
    status_str = "signed";
  } else {
    status_str = "unknown";
  }
  ...
  Profile* profile = Profile::FromBrowserContext(browser_context_);
  ExtensionId wootz_wallet_extension_id = GetWootzWalletExtensionId(profile);
  LOG(ERROR) << "jangid_sign: Extension ID for status change: "
             << wootz_wallet_extension_id;
  ...
  std::unique_ptr<Event> event = std::make_unique<Event>(
      ...
      "wootz.onTransactionStatusChangedAPI", std::move(event_args), profile,
      std::nullopt, GURL(), EventRouter::USER_GESTURE_UNKNOWN,
...
void WootzAPI::OnUnapprovedTxUpdated(
    wootz_wallet::mojom::TransactionInfoPtr tx_info) {
...
void WootzAPI::OnDropdownButtonClicked(const std::string& selectedFeature,
  ...
  dropdown_info.Set("timestamp",
                    base::Time::Now().InMillisecondsFSinceUnixEpoch());

  event_args.Append(std::move(dropdown_info));

  std::unique_ptr<Event> event = std::make_unique<Event>(
      events::WOOTZ_ON_DROPDOWN_BUTTON_CLICKED, "wootz.onDropdownButtonClicked",
      std::move(event_args), profile, std::nullopt, GURL(),
      EventRouter::USER_GESTURE_UNKNOWN, mojom::EventFilteringInfo::New());
  if (!event_router->ExtensionHasEventListener(
          extensionId, "wootz.onDropdownButtonClicked")) {
    LOG(WARNING) << "Extension is not listening for "
                    "wootz.onDropdownButtonClicked events: "
                 << extensionId;
  }
...
void WootzSignMessageFunction::NotifyExtensionOfPendingRequest(
  ...
  if (!service) {
    LOG(ERROR) << "JANGID: NotifyExtensionOfPendingRequest: Failed to get "
                  "WootzWalletService";
    return;
  }
...
void WootzSignMessageFunction::OnGetPendingRequests(
  ...
  if (requests.empty()) {
    LOG(ERROR)
        << "JANGID: OnGetPendingRequests: No pending requests, returning";
    return;
  }
  ...
  if (event_router) {
    LOG(ERROR)
        << "JANGID: OnGetPendingRequests: Broadcasting event to extension";
    base::Value::List event_args;
    ...
    LOG(ERROR) << "JANGID: Wootz Wallet extension ID: "
               << wootz_wallet_extension_id;
    ...
    if (!wootz_wallet_extension_id.empty()) {
      ...
      std::unique_ptr<Event> event = std::make_unique<Event>(
          ...
          "wootz.onSignMessageRequested", std::move(event_args), context,
          std::nullopt, GURL(), EventRouter::USER_GESTURE_UNKNOWN,
          mojom::EventFilteringInfo::New());
...
ExtensionFunction::ResponseAction WootzSignTransactionFunction::Run() {
  ...
  wootz_wallet::mojom::CoinType coin_type =
      static_cast<wootz_wallet::mojom::CoinType>(coin);
...
void WootzSignSolanaTransactionFunction::OnGetPendingRequests(
  ...
  request_dict.Set(
      "encodedMessage",
      base::Base64Encode(
          requests[0]->tx_data->get_solana_tx_data()->recent_blockhash));
  ...
  std::unique_ptr<Event> event = std::make_unique<Event>(
      ...
      "wootz.onSolanaSignTransactionRequested", std::move(event_args), context,
      std::nullopt, GURL(), EventRouter::USER_GESTURE_UNKNOWN,
      mojom::EventFilteringInfo::New());
...
ExtensionFunction::ResponseAction WootzSignSolanaTransactionFunction::Run() {
  ...
  service->NotifySignTransactionRequestProcessed(
      approved, request_id, std::move(signature_ptr), error);
...
ExtensionFunction::ResponseAction WootzGetBrowserInfoFunction::Run() {
  ...
  // This could be a combination of build date, channel, and other non-spoofable
  // information
...
//         Java_WootzBridge_showConsentDialog(env,
...
// }
// void
// WootzShowConsentDialogAndMaybeStartServiceFunction::OnConsentDialogResult(JNIEnv*
// env, jboolean consented) {
//     if (consented) {
//         Java_WootzBridge_startBrowsingDataService(env);
//     }
//     Respond(NoArguments());
...
ExtensionFunction::ResponseAction WootzGenerateZKProofFunction::Run() {
  ...
  if (args().size() != 2 || !args()[0].is_string() || !args()[1].is_string()) {
    LOG(ERROR)
        << "Invalid arguments received. Expected URL and content strings.";
    return RespondNow(
        Error("Invalid arguments. Expected URL and content strings."));
  }
  ...
  std::string keys_json = zk_proof::GenerateKeys(
      tls_data->cert_hash, tls_data->headers_json, content);
  ...
  std::string proof_json = zk_proof::GenerateProofWithKey(
      tls_data->cert_hash, tls_data->headers_json, content, proving_key);

  // Extract public inputs
  std::string public_inputs_json =
      zk_proof::ExtractPublicInputs(tls_data->cert_hash, content);
...
ExtensionFunction::ResponseAction WootzReplaceAdFunction::Run() {
  LOG(INFO) << "WootzReplaceAdFunction::Run started with arguments: "
            << args().size();

  // Validate arguments structure
  if (args().size() < 3 || !args()[0].is_bool() || !args()[1].is_dict() ||
      !args()[2].is_list()) {
  ...
  profile->GetPrefs()->SetBoolean(
      subresource_filter::prefs::kAdBlockGlobalEnabled, is_enabled);
  ...
  prefs->SetString(subresource_filter::prefs::kAdReplacementAdUnitPath,
                   *ad_unit_path);
  prefs->SetString(subresource_filter::prefs::kAdReplacementIdPrefix,
                   *id_prefix);
  prefs->SetString(subresource_filter::prefs::kAdReplacementScriptUrl,
                   *script_url);
  ...
  LOG(INFO) << "WootzReplaceAdFunction: Successfully stored ad replacement "
               "configuration";
...
ExtensionFunction::ResponseAction WootzGetPageStateFunction::Run() {
  ...
  if (args().size() >= 1 && args()[0].is_dict()) {
    ...
    is_background_web_contents =
        options.FindBool("isBackgroundWebContents").value_or(false);

    if (is_background_web_contents) {
      if (auto id = options.FindInt("backgroundWebContentsId")) {
        background_web_contents_id = id;
        web_contents =
            WebContentsIdToJavaWebContents(background_web_contents_id.value());
        if (!web_contents) {
          return RespondNow(Error("Background web contents not found"));
        }
        LOG(INFO) << "Successfully got background web contents with ID: "
                  << background_web_contents_id.value();
      } else {
        LOG(ERROR)
            << "Missing backgroundWebContentsId for background web contents";
        return RespondNow(
            Error("backgroundWebContentsId is required when "
                  "isBackgroundWebContents is true"));
      }
    }
  }
  ...
  auto* factory =
      automation::AutomationControllerFactory::FromWebContents(web_contents);
  if (!factory) {
  ...
  controller->GetPageState(
      ...
      base::BindOnce(&WootzGetPageStateFunction::OnGetPageStateComplete, this));
...
ExtensionFunction::ResponseAction WootzPerformActionFunction::Run() {
  ...
  bool is_background_web_contents =
      action_params.FindBool("isBackgroundWebContents").value_or(false);
  if (is_background_web_contents) {
    if (auto background_id = action_params.FindInt("backgroundWebContentsId")) {
      ...
      if (!web_contents) {
        LOG(ERROR) << "Background web contents not found with ID: "
                   << background_id.value();
        return RespondNow(Error("Background web contents not found"));
      }
    } else {
  ...
  controller->PerformAction(
      ...
      base::BindOnce(&WootzPerformActionFunction::OnActionComplete, this));
...
ExtensionFunction::ResponseAction
WootzCreateBackgroundWebContentsFunction::Run() {
  // Validate arguments
  ...
  Java_WootzAppBackgroundContentService_createBackgroundWebContents(
      env, webContentsId, base::android::ConvertUTF8ToJavaString(env, url));

  // Get the newly created WebContents
  content::WebContents* web_contents =
      WebContentsIdToJavaWebContents(webContentsId);
  if (web_contents) {
    ...
    LOG(INFO)
        << "Created AutomationControllerFactory for background WebContents ID: "
        << webContentsId;
  } else {
    LOG(ERROR) << "Failed to get WebContents after creation for ID: "
               << webContentsId;
  }
...
ExtensionFunction::ResponseAction
WootzDestroyBackgroundWebContentsFunction::Run() {
  ...
  base::Value::Dict result;
  result.Set("success", true);
  result.Set("message", "Background WebContents [" +
                            std::to_string(webContentsId) +
                            "] destroyed via Java service");
  return RespondNow(WithArguments(std::move(result)));
}

// ===== WootzMaskSensitiveElementsFunction =====

WootzMaskSensitiveElementsFunction::WootzMaskSensitiveElementsFunction() =
    default;
WootzMaskSensitiveElementsFunction::~WootzMaskSensitiveElementsFunction() =
    default;
...
ExtensionFunction::ResponseAction WootzMaskSensitiveElementsFunction::Run() {
  ...
  if (args().empty() || !args()[0].is_list()) {
    LOG(ERROR) << "[WootzAPI][Masking] Invalid arguments - expected array of "
                  "selectors";
  ...
  LOG(INFO) << "[WootzAPI][Masking] Got " << selectors.size()
            << " selectors to mask";
...
void WootzMaskSensitiveElementsFunction::OnMaskingComplete(int masked_count) {
  LOG(INFO) << "[WootzAPI][Masking] Masking complete: " << masked_count
            << " elements masked";
...
void WootzMaskSensitiveElementsFunction::SendSelectorsToRenderer(
    const std::vector<std::string>& selectors,
    int tab_id) {
  LOG(INFO) << "[WootzAPI][Masking] Sending " << selectors.size()
            << " selectors to renderer via Mojo:";
  for (const auto& selector : selectors) {
  ...
  if (tab_id != -1) {
    ...
    if (!ExtensionTabUtil::GetTabById(tab_id, browser_context(),
                                      ...
                                      &web_contents)) {
      LOG(ERROR) << "[WootzAPI][Masking] Failed to get WebContents for tab ID: "
                 << tab_id;
      OnMaskingComplete(0);
      return;
    }
    ...
    // Use the same approach as WootzReplaceElementFunction - get active tab
    // directly
    web_contents = TabModelList::GetCurrentTabModel()->GetActiveWebContents();
    if (!web_contents) {
    ...
    LOG(INFO)
        << "[WootzAPI][Masking] Targeting active tab (no tab ID provided)";
  }

  LOG(INFO) << "[WootzAPI][Masking] Targeting tab with URL: "
            << web_contents->GetVisibleURL().spec();

  // Use factory pattern like WootzReplaceElementFunction
  auto* factory =
      sensitive_masking::ContentSensitiveMaskingDriverFactory::FromWebContents(
          web_contents);
  if (!factory) {
    LOG(ERROR) << "[WootzAPI][Masking] ContentSensitiveMaskingDriverFactory "
                  "not available";
    OnMaskingComplete(0);
    return;
  }
  ...
  if (!driver) {
    LOG(ERROR)
        << "[WootzAPI][Masking] Failed to get masking driver for main frame";
    OnMaskingComplete(0);
    return;
  }

  // Send selectors using the factory's driver
  driver->UpdateMaskingSelectorsDirectly(
      selectors,
...
ExtensionFunction::ResponseAction
WootzChangeWootzAppSearchConfigurationFunction::Run() {
  ...
  TemplateURL* template_url =
      template_url_service->GetTemplateURLForKeyword(search_engine_keyword);
...
void JNI_WootzBridge_OnDropdownButtonClicked(
    JNIEnv* env,
    const base::android::JavaParamRef<jstring>& selectedFeature,
    const base::android::JavaParamRef<jstring>& extensionId,
    const base::android::JavaParamRef<jstring>& extensionName) {
  std::string feature =
      base::android::ConvertJavaStringToUTF8(env, selectedFeature);
  std::string extId = base::android::ConvertJavaStringToUTF8(env, extensionId);
  std::string extName =
      base::android::ConvertJavaStringToUTF8(env, extensionName);
  ...
  extensions::WootzAPI* wootz_api =
      extensions::WootzAPI::GetFactoryInstance()->Get(profile);
  if (!wootz_api) {
...
void JNI_WootzScreenshotApi_OnScreenshotComplete(
    JNIEnv* env,
    const base::android::JavaParamRef<jstring>& base64_data) {
  std::string base64_string =
      base::android::ConvertJavaStringToUTF8(env, base64_data);
  LOG(INFO) << "JNI: Screenshot completed, base64 length: "
  ...
  std::unique_ptr<extensions::Event> event =
      std::make_unique<extensions::Event>(
          ...
          "wootz.onScreenshotComplete", std::move(event_args), profile,
          std::nullopt, GURL(), extensions::EventRouter::USER_GESTURE_UNKNOWN,
...
void JNI_WootzScreenshotApi_OnScreenshotError(
    JNIEnv* env,
...
//     jboolean consented) {
//   auto* function =
//   reinterpret_cast<extensions::WootzShowConsentDialogAndMaybeStartServiceFunction*>(native_ptr);

...
content::WebUIDataSource* CreateAndAddBackgroundUIHTMLSource(Profile* profile) {
  content::WebUIDataSource* source = content::WebUIDataSource::CreateAndAdd(
      profile, chrome::kChromeUIBackgroundHost);
...
BackgroundUIConfig::BackgroundUIConfig()
    : WebUIConfig(content::kChromeUIScheme, chrome::kChromeUIBackgroundHost) {}

BackgroundUIConfig::~BackgroundUIConfig() = default;

std::unique_ptr<content::WebUIController>
BackgroundUIConfig::CreateWebUIController(content::WebUI* web_ui,
                                       const GURL& url) {
  Profile* profile = Profile::FromWebUI(web_ui);
  if (profile->IsGuestSession()) {
    return std::make_unique<PageNotAvailableForGuestUI>(
        web_ui, chrome::kChromeUIBackgroundHost);
  }
  return std::make_unique<BackgroundUI>(web_ui);
}

BackgroundUI::BackgroundUI(content::WebUI* web_ui)
    : ui::MojoWebUIController(web_ui, /*enable_chrome_send=*/true) {
  Profile* profile = Profile::FromWebUI(web_ui);
  content::WebUIDataSource* data_source =
      CreateAndAddBackgroundUIHTMLSource(profile);
  ManagedUIHandler::Initialize(web_ui, data_source);

  pref_change_registrar_.Init(profile->GetPrefs());
  pref_change_registrar_.Add(history_clusters::prefs::kVisible,
                             base::BindRepeating(&BackgroundUI::UpdateDataSource,
  ...
  web_ui->AddMessageHandler(
      std::make_unique<HistoryLoginHandler>(base::BindRepeating(
          &BackgroundUI::UpdateDataSource, base::Unretained(this))));
}

BackgroundUI::~BackgroundUI() = default;

WEB_UI_CONTROLLER_TYPE_IMPL(BackgroundUI)

// static
base::RefCountedMemory* BackgroundUI::GetFaviconResourceBytes(
...
void BackgroundUI::BindInterface(
...
void BackgroundUI::CreateShoppingServiceHandler(
...
void BackgroundUI::UpdateDataSource() {
  CHECK(web_ui());
  ...
  content::WebUIDataSource::Update(profile, chrome::kChromeUIBackgroundHost,
                                   std::move(update));
}
...
void BackgroundUI::CreateHelpBubbleHandler(

...
void AddBlockchainTokenImageSource(Profile* profile) {
  ...
  content::URLDataSource::Add(
      profile, std::make_unique<wootz_wallet::BlockchainImagesSource>(path));
}

bool IsWootzWalletOrigin(const url::Origin& origin) {
  return origin == url::Origin::Create(GURL(kWootzUIWalletPanelURL)) ||
         origin == url::Origin::Create(GURL(kWootzUIWalletPageURL));
}

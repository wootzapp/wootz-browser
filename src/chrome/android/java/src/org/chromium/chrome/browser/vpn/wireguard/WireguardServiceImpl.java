...
public class WireguardServiceImpl
    ...
    private static final int WOOTZ_VPN_NOTIFICATION_ID = 801;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        getService().startForeground(WOOTZ_VPN_NOTIFICATION_ID, getWootzVpnNotification(""));
    ...
    private Notification getWootzVpnNotification(String notificationText) {
        ...
        NotificationCompat.Builder notificationBuilder =
                new NotificationCompat.Builder(mContext, WootzActivity.CHANNEL_ID);
        notificationBuilder.setSmallIcon(R.drawable.ic_vpn)
                ...
                .setContentTitle(
                        String.format(mContext.getResources().getString(R.string.connected_to_host),
                                WootzVpnPrefUtils.getHostnameDisplay()))
    ...
    private void updateVpnNotification(String notificationText) {
        Notification notification = getWootzVpnNotification(notificationText);
        NotificationManager mNotificationManager =
                (NotificationManager) mContext.getSystemService(Context.NOTIFICATION_SERVICE);
        mNotificationManager.notify(WOOTZ_VPN_NOTIFICATION_ID, notification);
    }

    private void recordSessionTimes() {
        long sessionStartTimeMs = WootzVpnPrefUtils.getSessionStartTimeMs();
        long sessionEndTimeMs = WootzVpnPrefUtils.getSessionEndTimeMs();
        ...
        if (sessionStartTimeMs < 0) {
            WootzVpnPrefUtils.setSessionStartTimeMs(currTimeMs);
        }
        ...
        if (currDate.get(Calendar.YEAR) != sessionEndTimeCal.get(Calendar.YEAR)
            ...
            WootzVpnPrefUtils.setSessionEndTimeMs(currTimeMs);
        }
    }

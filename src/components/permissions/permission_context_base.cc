...
// --- Fix missing includes and macro confusion ---
#include <string_view>
...
// Remove macro that causes class name confusion in implementation
...
#define CanBypassEmbeddingOriginCheck(REQUESTING_ORIGIN, EMBEDDING_ORIGIN) \
  WootzCanBypassEmbeddingOriginCheck(REQUESTING_ORIGIN, EMBEDDING_ORIGIN,  \
...
bool IsGroupedPermissionType(ContentSettingsType type) {
  return type == ContentSettingsType::WOOTZ_ETHEREUM ||
...
// Only keep Wootz-specific helpers or classes here if needed.
// Remove duplicate PermissionDecided, DecidePermission, CleanUpRequest.

PermissionContextBase::GroupedPermissionRequests::GroupedPermissionRequests() = default;
PermissionContextBase::GroupedPermissionRequests::~GroupedPermissionRequests() = default;

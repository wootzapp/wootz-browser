...
interface Props {
  ...
  tokenNetwork?: WootzWallet.NetworkInfo
}
...
export const NftScreen = ({ selectedAsset, tokenNetwork }: Props) => {
  ...
  const onClickViewOnBlockExplorer = useExplorer(
    tokenNetwork || new WootzWallet.NetworkInfo()
  )
  ...
  const onClickContractAddress = React.useCallback(() => {
    onClickViewOnBlockExplorer(
      selectedAsset.coin === WootzWallet.CoinType.ETH ? 'nft' : 'token',
  ...
  if (nftMetadataError)
    return (
      <StyledWrapper>
        ...
        >
          <ErrorMessage>
            {getLocale('wootzWalletNftFetchingError')}
          </ErrorMessage>
        </Row>
      </StyledWrapper>
    )
  ...
  return (
    <StyledWrapper>
      ...
      <SectionTitle>{getLocale('wootzWalletNFTDetailsOverview')}</SectionTitle>
      {ownerAddress && (
        <SectionWrapper>
          <InfoBox>
            <InfoTitle>{getLocale('wootzWalletNFTDetailsOwnedBy')}</InfoTitle>
            <Row
            ...
            {ownerAccount ? (
              <ViewAccount onClick={() => onClickViewAccount(ownerAccount)}>
                {getLocale('wootzWalletNFTDetailsViewAccount')}
              </ViewAccount>
            ) : (
              <ViewAccount
      ...
      <SectionWrapper>
        {selectedAsset.tokenId !== '' && (
          <InfoBox>
            <InfoTitle>{getLocale('wootzWalletNFTDetailTokenID')}</InfoTitle>
            <Row
              justifyContent='flex-start'
              gap='4px'
            >
        ...
        {tokenNetwork && (
          <InfoBox>
            <InfoTitle>{getLocale('wootzWalletNFTDetailBlockchain')}</InfoTitle>
      ...
      <SectionWrapper>
        ...
        <InfoBox>
          <InfoTitle>
            {getLocale('wootzWalletNFTDetailTokenStandard')}
          </InfoTitle>
      ...
      <SectionWrapper>
        {isFetchingNFTMetadata ? (
          ...
          <InfoBox>
            <InfoTitle>
              {getLocale('wootzWalletNFTDetailDescription')}
            </InfoTitle>
            <Description>
      ...
      {isFetchingNFTMetadata ? (
        ...
        <>
          {nftMetadata?.attributes?.length !== 0 && (
            <SectionTitle>
              {getLocale('wootzWalletNFTDetailsProperties')}
            </SectionTitle>
          )}
        </>
      )}

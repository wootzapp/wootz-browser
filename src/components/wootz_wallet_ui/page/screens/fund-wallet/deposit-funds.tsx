...
function AssetSelection() {
  ...
  const { nftCollectionAssets, nftCollectionAssetsIds } = React.useMemo(() => {
    const nftCollectionAssets: WootzWallet.BlockchainToken[] = []
  ...
  const fullAssetsList: WootzWallet.BlockchainToken[] = React.useMemo(() => {
  ...
  const renderToken = React.useCallback<
    RenderTokenFunc<WootzWallet.BlockchainToken>
  >(
    ({ item: asset, ref }) => {
...
function DepositAccount() {
  ...
  const depositTitleText: string = React.useMemo(() => {
    ...
    const isFil = selectedAsset?.coin === WootzWallet.CoinType.FIL
    const isSolOrSpl = selectedAsset?.coin === WootzWallet.CoinType.SOL
    const isErc = selectedAsset?.isErc20 || selectedAsset?.isErc721

    // EVM native network (gas) assets & Filecoin
    if (isNativeAsset || isFil) {
      return getLocale('wootzWalletDepositX').replace(
        '$1',
    ...
    // ERC-based tokens
    if (isErc) {
      return getLocale('wootzWalletDepositErc')
    }

    // Solana assets
    if (isSolOrSpl) {
      return getLocale('wootzWalletDepositSolSplTokens')
    }
    ...
    >
      <Column alignItems='flex-start'>
        ...
        {selectedAssetNetwork && (
          <Description>
            {getLocale('wootzWalletDepositOnlySendOnXNetwork').replace(
              '$1',

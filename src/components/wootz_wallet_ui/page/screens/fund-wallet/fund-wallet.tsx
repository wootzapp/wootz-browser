...
function AssetSelection({ isAndroid }: Props) {
  ...
  const renderToken: RenderTokenFunc<WootzWallet.BlockchainToken> =
    React.useCallback(
      ({ item: asset, ref }) => {
  ...
  const networksFilterOptions: WootzWallet.NetworkInfo[] = React.useMemo(() => {
    return [AllNetworksOption, ...buyAssetNetworks]
  }, [buyAssetNetworks])

  // computed
  const isNextStepEnabled = !!selectedAsset
  const pageTitle = `${getLocale('wootzWalletBuy')}${
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {selectedAsset
              ? getLocale('wootzWalletBuyContinueButton')
              : getLocale('wootzWalletBuySelectAsset')}
          </LeoSquaredButton>
        </Row>
      </StyledWrapper>
    </WalletPageWrapper>
...
function PurchaseOptionSelection({ isAndroid }: Props) {
  ...
  const { data: buyWithStripeUrl } = useGetBuyUrlQuery(
    selectedAsset && assetNetwork && generatedAddress
      ? {
          ...
          onRampProvider: WootzWallet.OnRampProvider.kStripe,
  ...
  const onRampAssetMap = React.useMemo(() => {
    ...
    return {
      [WootzWallet.OnRampProvider.kRamp]: options.rampAssetOptions,
      [WootzWallet.OnRampProvider.kSardine]: options.sardineAssetOptions,
      [WootzWallet.OnRampProvider.kTransak]: options.transakAssetOptions,
      [WootzWallet.OnRampProvider.kStripe]: options.stripeAssetOptions,
      [WootzWallet.OnRampProvider.kCoinbase]: options.coinbaseAssetOptions
    }
  }, [options])
  ...
  const selectedAssetBuyOptions: BuyOption[] = React.useMemo(() => {
    ...
    return selectedAsset
      ? BuyOptions.filter((buyOption) =>
          ...
          .filter((buyOption) => {
            if (buyOption.id === WootzWallet.OnRampProvider.kStripe) {
  ...
  const pageTitle = selectedAsset
    ? `${getLocale('wootzWalletBuy')} ${selectedAsset.symbol}`
    : getLocale('wootzWalletBuy')
  ...
  const openBuyAssetLink = React.useCallback(
    async (buyOption: WootzWallet.OnRampProvider) => {
      ...
      try {
        const url = await getBuyUrl({
          assetSymbol:
            buyOption === WootzWallet.OnRampProvider.kRamp
              ? getRampAssetSymbol(selectedAsset)
              : buyOption === WootzWallet.OnRampProvider.kStripe
    ...
    >
      ...
      >
        {needsAccount && assetNetwork ? (
          ...
          <SearchWrapper>
            <SelectHeader
              title={getLocale('wootzWalletSelectAccount')}
              onBack={closeAccountSearch}
              hasAddButton={false}
            />
            <SearchBar
              placeholder={getLocale('wootzWalletSearchAccount')}
          ...
          <>
            <Title>
              {getLocale('wootzWalletFundWalletTitle').replace(
                '$1',
                selectedAsset?.symbol ?? ''
              )}
            </Title>
            ...
            <Description>
              {getLocale('wootzWalletFundWalletDescription')}
            </Description>
            ...
            <Alert>
              ...
              <AlertText>{getLocale('wootzWalletBuyDisclaimer')}</AlertText>
            </Alert>
          </>
        )}
      </Column>

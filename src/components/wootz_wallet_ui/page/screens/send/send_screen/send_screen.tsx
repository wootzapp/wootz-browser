...
export const SendScreen = React.memo((props: Props) => {
  ...
  const selectSendAsset = React.useCallback(
    (asset: WootzWallet.BlockchainToken, account?: WootzWallet.AccountInfo) => {
      ...
      case WootzWallet.CoinType.ETH: {
        ...
        if (
          (tokenFromParams.chainId ===
            WootzWallet.FILECOIN_ETHEREUM_MAINNET_CHAIN_ID ||
            tokenFromParams.chainId ===
    ...
    <>
      <WalletPageWrapper
        ...
        cardHeader={
          isPanel ? (
            <PanelActionHeader
              title={getLocale('wootzWalletSend')}
          ...
          >
            ...
            >
              ...
              >
                ...
                {tokenFromParams?.coin === WootzWallet.CoinType.BTC && (
                  <OrdinalsWarningMessage
...
function ethToWeiAmount(
  sendAmount: string,
  selectedSendAsset: WootzWallet.BlockchainToken
): Amount {
...
function getReviewButtonText(
  ...
  if (sendAmountValidationError) {
    return 'wootzWalletDecimalPlacesError'
  }
  if (insufficientFundsError) {
    return 'wootzWalletNotEnoughFunds'
  }

  return 'wootzWalletReviewSend'
}

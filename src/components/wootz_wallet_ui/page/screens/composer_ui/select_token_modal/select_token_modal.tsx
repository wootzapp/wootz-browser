...
// Types
import { WootzWallet, SendPageTabHashes } from '../../../../constants/types'
import {
...
interface Props {
  ...
  selectedFromToken?: WootzWallet.BlockchainToken
  selectedToToken?: WootzWallet.BlockchainToken
...
export const SelectTokenModal = React.forwardRef<HTMLDivElement, Props>(
  (props: Props, forwardedRef) => {
    ...
    const [selectedNetworkFilter, setSelectedNetworkFilter] =
      React.useState<WootzWallet.NetworkInfo>(
        selectedNetwork || AllNetworksOption
      )
    const [selectedAccountFilter, setSelectedAccountFilter] =
      React.useState<WootzWallet.AccountInfo>(AllAccountsOption)
    ...
    const { data: spotPriceRegistry, isLoading: isLoadingSpotPrices } =
      useGetTokenSpotPricesQuery(
        !isLoadingBalances &&
          ...
          ? {
              ids: tokenPriceIds,
              timeframe: WootzWallet.AssetPriceTimeframe.OneDay,
    ...
    const checkIsAccountOptionDisabled = React.useCallback(
      (account: WootzWallet.AccountInfo) => {
        ...
        if (
          ...
          return (
            account.accountId.coin === WootzWallet.CoinType.SOL &&
            selectedFromToken.coin === WootzWallet.CoinType.SOL
          )
        }
        if (
          modalType === 'bridge' &&
          ...
          return (
            account.accountId.coin === WootzWallet.CoinType.SOL &&
            selectedToToken.coin === WootzWallet.CoinType.SOL
          )
        }
        return false
      },
      ...
      if (emptyTokensList) {
        return (
          <NoAssetsText
            ...
            isBold={false}
          >
            {getLocale('wootzWalletNoAvailableTokens')}
          </NoAssetsText>
        )
      }
    ...
    const swapSupportedAccounts = React.useMemo(() => {
      return accounts.filter(
        (account) =>
          account.accountId.coin === WootzWallet.CoinType.ETH ||
          account.accountId.coin === WootzWallet.CoinType.SOL
      )
    }, [accounts])
    ...
    return (
      <>
        <PopupModal
          ...
          title={getLocale(
            modalType === 'swap'
              ? 'wootzWalletChooseAssetToSwap'
              : modalType === 'bridge'
              ? 'wootzWalletChooseAssetToBridge'
              : 'wootzWalletChooseAssetToSend'
          )}
          width='560px'
          height='90vh'
            ...
            <SearchInput
              ...
              placeholder={
                selectedSendOption === SendPageTabHashes.nft
                  ? getLocale('wootzWalletSearchNFTs')
                  : getLocale('wootzWalletSearchTokens')
              }
              type='text'
            >
              <div slot='left-icon'>

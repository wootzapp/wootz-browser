...
export const FromAsset = (props: Props) => {
  ...
  const { data: bitcoinBalances, isLoading: isLoadingBitcoinBalances } =
    useGetBitcoinBalancesQuery(
      token && token?.coin === WootzWallet.CoinType.BTC && account?.accountId
        ? account.accountId
        : skipToken
    )
  ...
  const tokenBalance = React.useMemo(() => {
    ...
    if (token.coin === WootzWallet.CoinType.BTC && bitcoinBalances) {
        ...
        >
          {isLoadingBalances || isLoadingBitcoinBalances ? (
            ...
            <>
              ...
              {token.coin === WootzWallet.CoinType.BTC && hasPendingBalance && (
                <>
                  <BalanceText
                    textSize='14px'
                    isBold={true}
          ...
          {token && !token.isNft && account !== undefined && (
            <Row width='unset'>
              <PresetButton onClick={() => setPresetAmountValue(0.5)}>
                {getLocale('wootzWalletSendHalf')}
              </PresetButton>
              <HorizontalSpace space='8px' />
              <PresetButton onClick={() => setPresetAmountValue(1)}>
                {getLocale('wootzWalletSendMax')}
              </PresetButton>
            </Row>
          )}
        </AccountNameAndPresetsRow>
        ...
        >
          <Row width='unset'>
            <SelectButton
              ...
              placeholderText={getLocale('wootzWalletChooseAsset')}
            />
          </Row>
          {!token?.isNft && (
            <AmountInput

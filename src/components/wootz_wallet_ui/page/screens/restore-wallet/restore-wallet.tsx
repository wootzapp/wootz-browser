...
export const RestoreWallet = () => {
  ...
  return (
    <>
      ...
      <StyledWrapper>
        <Title>{getLocale('wootzWalletRestoreTite')}</Title>
        <Description>{getLocale('wootzWalletRestoreDescription')}</Description>

        <FormWrapper>
          <RecoveryPhraseInput
            autoFocus={true}
            placeholder={getLocale('wootzWalletRestorePlaceholder')}
          ...
          {invalidMnemonic && (
            <ErrorText>{getLocale('wootzWalletRestoreError')}</ErrorText>
          )}
            ...
            <LegacyCheckboxRow>
              ...
              >
                <div data-key='isLegacy'>
                  {getLocale('wootzWalletRestoreLegacyCheckBox')}
                </div>
              </Checkbox>
            </LegacyCheckboxRow>
          )}
          ...
          <CheckboxRow>
            ...
            >
              <div data-key='showPhrase'>
                {getLocale('wootzWalletRestoreShowPhrase')}
              </div>
            </Checkbox>
          </CheckboxRow>

          <FormText>{getLocale('wootzWalletRestoreFormText')}</FormText>
          <Description textAlign='left'>
            {getLocale('wootzWalletCreatePasswordDescription')}
          </Description>
          <InputColumn>
            <PasswordInput
              placeholder={getLocale('wootzWalletCreatePasswordInput')}
              onChange={handlePasswordChanged}
              hasError={hasPasswordError}
              error={getLocale('wootzWalletCreatePasswordError')}
              onKeyDown={handleKeyDown}
            />
            <PasswordInput
              placeholder={getLocale('wootzWalletConfirmPasswordInput')}
              onChange={handleConfirmPasswordChanged}
              hasError={hasConfirmedPasswordError}
              error={getLocale('wootzWalletConfirmPasswordError')}
              onKeyDown={handleKeyDown}
            />
          </InputColumn>
        </FormWrapper>
        ...
        <NavButton
          disabled={isDisabled}
          buttonType='primary'
          text={getLocale('wootzWalletWelcomeRestoreButton')}

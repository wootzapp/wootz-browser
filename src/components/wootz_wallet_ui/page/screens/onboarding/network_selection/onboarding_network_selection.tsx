...
export const OnboardingNetworkSelection = () => {
  ...
  const { featuredNetworks, popularNetworks } = React.useMemo(() => {
    ...
    const featuredNetworks = featuredChainIds
      ...
      .filter((net): net is WootzWallet.NetworkInfo => Boolean(net))
  ...
  const onSubmit = React.useCallback(async () => {
    ...
    > = []
    const hiddenNets: Array<Pick<WootzWallet.NetworkInfo, 'chainId' | 'coin'>> =
      []
  ...
  return (
    <OnboardingContentLayout
      title={getLocale('wootzWalletSupportedNetworks')}
      subTitle={getLocale('wootzWalletChooseChainsToUse')}
      padding='44px 0 0'
    >
      <Column
        fullWidth
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {getLocale('wootzWalletShowTestnets')}
            </Text>
          </Checkbox>
        </Row>
          ...
          <ChainSelectionContextProvider value={selectedChainsContext}>
            {isLoadingNetworks ? (
              ...
              <>
                ...
                >
                  <GroupingText>{getLocale('wootzWalletPopular')}</GroupingText>
                  {networks && (
                    <SelectAllText
                      onClick={() => {
                    ...
                    >
                      {getLocale(
                        areAllChainsSelected
                          ? 'wootzWalletDeselectAll'
                          : 'wootzWalletSelectAll'
                      )}
                    </SelectAllText>
                  )}
                </Row>
      ...
      <NextButtonRow>
        ...
        >
          {visibleSelectedChainIds.length
            ? getLocale('wootzWalletContinueWithXItems')
                .replace(
                  '$1', // Number of items
      ...
      <MutedLinkText onClick={openNetworkSettings}>
        {getLocale('wootzWalletAddNetworksAnytimeInSettings')}
      </MutedLinkText>
    </OnboardingContentLayout>
  )
}

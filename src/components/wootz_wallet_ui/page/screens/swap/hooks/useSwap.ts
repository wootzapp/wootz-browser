...
export const useSwap = () => {
  ...
  const [quoteUnion, setQuoteUnion] = useState<
    WootzWallet.SwapQuoteUnion | undefined
  >(undefined)
  const [quoteErrorUnion, setQuoteErrorUnion] = useState<
    WootzWallet.SwapErrorUnion | undefined
  >(undefined)
  ...
  const [swapFees, setSwapFees] = useState<WootzWallet.SwapFees | undefined>(
    undefined
  )
  ...
  const [selectedProvider, setSelectedProvider] =
    useState<WootzWallet.SwapProvider>(WootzWallet.SwapProvider.kAuto)
  ...
  const tokenPriceIds = useMemo(
    () =>
      [nativeAsset, fromToken, toToken]
        .filter(
          (token): token is WootzWallet.BlockchainToken => token !== undefined
        )
    ...
    async (overrides: SwapParamsOverrides) => {
      ...
      try {
        quoteResponse = await generateSwapQuote({
          ...
          routePriority: WootzWallet.RoutePriority.kCheapest,
  ...
  const availableProvidersForSwap = useMemo(() => {
    ...
    if (fromToken && fromToken.coin === WootzWallet.CoinType.SOL) {
  ...
  const swapValidationError: SwapValidationErrorType | undefined =
    useMemo(() => {
      ...
      if (
        ...
        fromToken.coin === WootzWallet.CoinType.ETH &&
        fromToken.contractAddress &&
        !hasAllowance
      ) {
      ...
      if (
        ...
        quoteErrorUnion?.lifiError?.code !== WootzWallet.LiFiErrorCode.kSuccess
      ) {
        return quoteErrorUnion?.lifiError.code ===
          WootzWallet.LiFiErrorCode.kNotFoundError
  ...
  const onChangeSwapProvider = useCallback(
    async (provider: WootzWallet.SwapProvider) => {
  ...
  const submitButtonText = useMemo(() => {
    if (isFetchingQuote) {
      return getLocale('wootzWalletFetchingQuote')
    }

    const defaultText = isBridge
      ? getLocale('wootzWalletReviewBridge')
      : getLocale('wootzWalletReviewSwap')
    ...
    if (swapValidationError === 'insufficientBalance') {
      return getLocale('wootzSwapInsufficientBalance').replace(
        '$1',
        fromToken.symbol
      )
    }
    ...
    if (
      fromNetwork.coin === WootzWallet.CoinType.ETH &&
      swapValidationError === 'insufficientAllowance'
    ) {
      return getLocale('wootzSwapApproveToken').replace('$1', fromToken.symbol)
    }

    if (swapValidationError === 'insufficientLiquidity') {
      return getLocale('wootzSwapInsufficientLiquidity')
    }

    if (swapValidationError === 'unknownError') {
      return getLocale('wootzWalletSwapUnknownError')
    }
  ...
  const isSubmitButtonDisabled = useMemo(() => {
    return (
      ...
      (swapValidationError &&
        fromNetwork.coin === WootzWallet.CoinType.ETH &&
        swapValidationError !== 'insufficientAllowance') ||
      (swapValidationError && fromNetwork.coin === WootzWallet.CoinType.SOL)
    )
  }, [
    fromNetwork,
    fromAccount,

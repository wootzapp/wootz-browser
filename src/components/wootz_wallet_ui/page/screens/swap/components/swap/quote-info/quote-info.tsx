...
interface Props {
  fromToken: WootzWallet.BlockchainToken | undefined
  toToken: WootzWallet.BlockchainToken | undefined
  isBridge: boolean
  ...
  toAccount?: WootzWallet.AccountInfo
  swapFees?: WootzWallet.SwapFees
}
...
export const QuoteInfo = (props: Props) => {
  ...
  const swapRate: string = React.useMemo(() => {
    ...
    return getLocale('wootzWalletExchangeFor')
  ...
  const coinGeckoDeltaText: string = React.useMemo(() => {
    if (coinGeckoDelta.gte(0)) {
      return getLocale('wootzSwapCoinGeckoCheaper').replace(
        '$1',
    ...
    if (coinGeckoDelta.gte(-1)) {
      return getLocale('wootzSwapCoinGeckoWithin').replace(
        '$1',
        coinGeckoDelta.times(-1).format(2)
      )
    }

    return getLocale('wootzSwapCoinGeckoExpensive').replace(
      '$1',
      coinGeckoDelta.times(-1).format(2)
    )
  }, [coinGeckoDelta])
  ...
  const wootzFee: WootzWallet.SwapFees | undefined = React.useMemo(() => {
    if (!swapFees) {
      return undefined
    }
  ...
  const effectiveFeeAmount = wootzFee && new Amount(wootzFee.effectiveFeePct)
  const firstStep =
    selectedQuoteOption?.sources.find((source) =>
      source.includedSteps?.some(
        (step) => step.type === WootzWallet.LiFiStepType.kCross
      )
  ...
  const { result: exchangeStepsLocale } = usePromise(
    async () =>
      PluralStringProxyImpl.getInstance().getPluralString(
        'wootzWalletExchangeNamePlusSteps',
      ...
      >
        {minimumReceived !== '' && (
          <Row justifyContent='space-between'>
            <Text
              ...
              textAlign='left'
            >
              {getLocale('wootzSwapMinimumReceivedAfterSlippage')}
            </Text>
            <Text
              textSize='12px'
              isBold={true}
        ...
        <Row justifyContent='space-between'>
          <Text
            ...
            textAlign='left'
          >
            {getLocale('wootzWalletMaxSlippage')}
          </Text>
          <Row width='unset'>
            <Text
              textSize='12px'
            ...
            {!isBridge && (
              <Row justifyContent='space-between'>
                <Text
                  ...
                  textColor='secondary'
                >
                  {getLocale('wootzWalletExchangeRate')}
                </Text>
                <Text
                  textSize='12px'
                  isBold={true}
            ...
            <Row justifyContent='space-between'>
              ...
              >
                <Text
                  ...
                  textAlign='left'
                >
                  {getLocale('wootzSwapPriceImpact')}
                </Text>
                <InfoIconTooltip
                  placement='right'
                  text={getLocale('wootzWalletPriceImpactDescription')}
            ...
            {isBridge && (
              <Row justifyContent='space-between'>
                <Text
                  ...
                  textAlign='left'
                >
                  {getLocale('wootzWalletRecipient')}
                </Text>
                  ...
                  {effectiveFeeAmount && effectiveFeeAmount.isZero() ? (
                    <Bubble padding='5px 6px'>
                      <FreeText
                        ...
                        isBold={true}
                      >
                        {getLocale('wootzSwapFree')}
                      </FreeText>
                    </Bubble>
                  ) : (
                    <Text textSize='14px'>{wootzFee.effectiveFeePct}%</Text>
                  )}

                  {wootzFee.discountCode !==
                    WootzWallet.SwapDiscountCode.kNone && (
                    <>
                      <WootzFeeDiscounted
                        textSize='12px'
                        isBold={true}

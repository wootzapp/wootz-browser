    ...
    >
      ...
      >
        <Text
          ...
          textAlign='left'
        >
          {step.type === WootzWallet.LiFiStepType.kSwap
            ? getLocale('wootzWalletSwapOnNetworkViaExchange')
                .replace('$1', fromNetwork?.chainName ?? '')
                .replace('$2', toolDetails.name)
            : getLocale('wootzWalletBridgeFromNetworkToNetwork')
                .replace('$1', fromNetwork?.chainName ?? '')

...
const GetBalanceSection = (props: GetBalanceSectionProps) => {
  ...
  const [balance, setBalance] = useState<
    WootzWallet.BitcoinBalance | undefined
  >()
...
> = ({ accountId }) => {
  ...
  const [bitcoinAccountInfo, setBitcoinAccountInfo] = useState<
    WootzWallet.BitcoinAccountInfo | undefined
  >()
  ...
  const keyId = (keyId: WootzWallet.BitcoinKeyId | undefined) => {
    if (!keyId) {
      return '-'
    }
...
interface AccountSectionProps {
  accountInfo: WootzWallet.AccountInfo
}
...
export const DevBitcoin = () => {
  const [accounts, setAccounts] = useState<WootzWallet.AccountInfo[]>([])
    ...
    const fetchBitcoinAccount = async () => {
      ...
      setAccounts(
        allAccounts.accounts.filter(
          (acc) => acc.accountId.coin === WootzWallet.CoinType.BTC
        )
      )
    }

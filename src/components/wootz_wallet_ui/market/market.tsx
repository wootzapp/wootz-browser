...
const App = () => {
  ...
  // Constants
  const origin =
    window.location.ancestorOrigins[0] === wootzWalletPanelOrigin
      ? wootzWalletPanelOrigin
      : wootzWalletOrigin
  ...
  const isBuySupported = React.useCallback(
    (coinMarket: WootzWallet.CoinMarket) => {
  ...
  const onMessageEventListener = React.useCallback(
    (event: MessageEvent<MarketCommandMessage>) => {
      ...
      if (
        event.origin === wootzWalletOrigin ||
        event.origin === wootzWalletPanelOrigin
      ) {

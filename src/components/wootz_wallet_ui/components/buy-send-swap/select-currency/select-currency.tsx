...
export interface Props {
  onSelectCurrency: (currency: WootzWallet.OnRampCurrency) => void
  onBack: () => void
}
...
export const SelectOnRampFiatCurrency = (props: Props) => {
  ...
  return (
    <SelectCurrencyWrapper>
      <Header
        title={getLocale('wootzWalletSelectCurrency')}
        onBack={onBack}
        hasAddButton={false}
      />
      <SearchBar
        placeholder={getLocale('wootzWalletSearchCurrency')}
      ...
      <SelectScrollSearchContainer>
        {isLoading ? (
          ...
          filteredCurrencies.map((currency: WootzWallet.OnRampCurrency) => (
            <SelectCurrencyItem

...
export type MarketGridProps = {
  ...
  coinMarketData: WootzWallet.CoinMarket[]
  visibleRows?: number
  rowHeight?: number
  ...
  onSelectCoinMarket: (coinMarket: WootzWallet.CoinMarket) => void
  isBuySupported: (coinMarket: WootzWallet.CoinMarket) => boolean
  isDepositSupported: (coinMarket: WootzWallet.CoinMarket) => boolean
  onClickBuy: (coinMarket: WootzWallet.CoinMarket) => void
  onClickDeposit: (coinMarket: WootzWallet.CoinMarket) => void
...
export const MarketGrid = ({
  ...
  const rows: MarketGridRow[] = React.useMemo(() => {
    return coinMarketData.map((coinMarketItem: WootzWallet.CoinMarket) => {
  ...
  return (
    ...
    >
        ...
        {showEmptyState ? (
          <Row margin='30px 0px'>
            <EmptyStateText
              ...
              textSize='14px'
            >
              {getLocale('wootzWalletMarketDataNoAssetsFound')}
            </EmptyStateText>
          </Row>
        ) : (
          <FixedSizeList
      ...
      <CoinGeckoText>
        {getLocale('wootzWalletPoweredByCoinGecko')}
      </CoinGeckoText>
    </StyledWrapper>
  )
}

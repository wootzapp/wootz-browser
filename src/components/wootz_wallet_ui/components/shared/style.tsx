...
export const StatusBubble = styled.div<{
  ...
  border-radius: 100%;
  opacity: ${(p) =>
    p.status === WootzWallet.TransactionStatus.Submitted ||
    p.status === WootzWallet.TransactionStatus.Approved ||
    p.status === WootzWallet.TransactionStatus.Unapproved ||
    p.status === WootzWallet.TransactionStatus.Signed
      ? 0.4
      : 1};
  background-color: ${(p) =>
    p.status === WootzWallet.TransactionStatus.Confirmed ||
    p.status === WootzWallet.TransactionStatus.Approved
      ? p.theme.color.successBorder
      : p.status === WootzWallet.TransactionStatus.Rejected ||
        p.status === WootzWallet.TransactionStatus.Error ||
        p.status === WootzWallet.TransactionStatus.Dropped
...
export const WalletWelcomeGraphic = styled.div<{
  ...
  width: 350px;
  height: 264px;
  background: url(${WootzWalletWithCoins});
...
export const WootzRewardsIndicator = styled.div`
  display: flex;
  align-items: center;
  justify-content: center;
  font-family: Poppins;

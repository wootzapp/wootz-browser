...
export type Props =
  | {
      addChainRequest: WootzWallet.AddChainRequest
      switchChainRequest?: never
    }
  | {
      switchChainRequest: WootzWallet.SwitchChainRequest
...
const onLearnMore = () => {
  chrome.tabs
    .create({
      url:
        'https://support.wootz.com' +
        '/hc/en-us/articles/4415497656461-Wootz-Wallet-FAQ'
    })
    .catch((e) => {
      console.error(e)
    })
  ...
  return (
    <StyledWrapper>
      <CenterColumn>
        ...
        <PanelTitle>
          {switchChainRequest
            ? getLocale('wootzWalletAllowChangeNetworkTitle')
            : getLocale('wootzWalletAllowAddNetworkTitle')}
        </PanelTitle>
        <Description>
          {switchChainRequest
            ? getLocale('wootzWalletAllowChangeNetworkDescription')
            : getLocale('wootzWalletAllowAddNetworkDescription')}{' '}
          {addChainRequest && (
        ...
        <TabRow>
          <PanelTab
            ...
            text={getLocale('wootzWalletAllowAddNetworkNetworkPanelTitle')}
          />
          <PanelTab
        ...
        <MessageBox>
          <MessageBoxColumn>
            <NetworkTitle>
              {getLocale('wootzWalletAllowAddNetworkName')}
            </NetworkTitle>
            <NetworkDetail>{network?.chainName}</NetworkDetail>
          </MessageBoxColumn>
          <MessageBoxColumn>
            <NetworkTitle>
              {getLocale('wootzWalletAllowAddNetworkUrl')}
            </NetworkTitle>
          ...
          {selectedTab === 'details' && (
            <>
              <MessageBoxColumn>
                <NetworkTitle>{getLocale('wootzWalletChainId')}</NetworkTitle>
                <NetworkDetail>{network?.chainId}</NetworkDetail>
              </MessageBoxColumn>
              <MessageBoxColumn>
                <NetworkTitle>
                  {getLocale('wootzWalletAllowAddNetworkCurrencySymbol')}
                </NetworkTitle>
                <NetworkDetail>{network?.symbol}</NetworkDetail>
              </MessageBoxColumn>
              <MessageBoxColumn>
                <NetworkTitle>
                  {getLocale('wootzWalletWatchListTokenDecimals')}
                </NetworkTitle>
                <NetworkDetail>{network?.decimals}</NetworkDetail>
              </MessageBoxColumn>
              <MessageBoxColumn>
                <NetworkTitle>
                  {getLocale('wootzWalletAllowAddNetworkExplorer')}
                </NetworkTitle>
      ...
      <ButtonRow>
        ...
        <NavButton
          ...
          text={
            switchChainRequest
              ? getLocale('wootzWalletAllowChangeNetworkButton')
              : getLocale('wootzWalletAllowAddNetworkButton')
          }
          onSubmit={

...
const onClickInstructions = () => {
  const url = 'https://support.wootz.com/hc/en-us/articles/4409309138701'
...
function getAppName(coinType: WootzWallet.CoinType): string {
  switch (coinType) {
...
export const ConnectHardwareWalletPanel = ({
  ...
  const title = React.useMemo(() => {
    if (hardwareWalletCode === 'deviceBusy') {
      return getLocale('wootzWalletConnectHardwarePanelConfirmation')
    }
    ...
    if (
      ...
      return getLocale('wootzWalletConnectHardwarePanelConnect').replace(
        '$1',
        account.name
      )
    }
    ...
    return getLocale('wootzWalletConnectHardwarePanelOpenApp')
      .replace('$1', network)
  ...
  React.useEffect(() => {
    ...
    // back to wootz://wallet-panel.top-chrome/ without any
  ...
  return (
    <StyledWrapper>
      <ConnectionRow>
        ...
        <Description>
          {isConnected
            ? getLocale('wootzWalletConnectHardwarePanelConnected').replace(
                '$1',
                account.name
              )
            : getLocale('wootzWalletConnectHardwarePanelDisconnected').replace(
                '$1',
                account.name
              )}
        </Description>
      ...
      <InstructionsButton onClick={onClickInstructions}>
        {getLocale('wootzWalletConnectHardwarePanelInstructions')}
      </InstructionsButton>
      <PageIcon />

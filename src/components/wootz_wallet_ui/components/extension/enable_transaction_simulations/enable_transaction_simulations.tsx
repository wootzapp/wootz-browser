...
const TX_SIMULATION_LEARN_MORE_LINK =
  'https://github.com/wootz/wootz-browser/wiki/Transaction-Simulation'
...
export const EnableTransactionSimulations: React.FC = () => {
  ...
  return (
    <>
      <Title>{getLocale('wootzWalletEnableTransactionSimulation')}</Title>
      <CardContent>
        <Column>
          <Row
            width='100%'
          ...
          >
            <IconContainer>
              <Icon name={'product-wootz-wallet'} />
            </IconContainer>
            <DashedHorizontalLine />
          ...
          <div>
            <HeadingText>
              {getLocale('wootzWalletTransactionSimulationFeatureDescription')}
            </HeadingText>

            <BulletPoints>
              <li key={'estimate'}>
                {getLocale('wootzWalletTransactionSimulationSeeEstimates')}
              </li>
              <li key={'detect'}>
                {getLocale('wootzWalletTransactionSimulationDetectMalicious')}
              </li>
              <li key={'phishing'}>
                {getLocale('wootzWalletTransactionSimulationDetectPhishing')}
                {
                  //
                  ' '
                }
                ...
                >
                  {getLocale('wootzWalletLearnMore')}
                </a>
              </li>
            </BulletPoints>
          ...
          <OptionsRow>
            <LeoSquaredButton
              ...
              onClick={async () => {
                await optInOrOut(
                  WootzWallet.BlowfishOptInStatus.kDenied
                ).unwrap()
              }}
            >
              {getLocale('wootzWalletButtonNoThanks')}
            </LeoSquaredButton>
            <LeoSquaredButton
              kind='filled'
              onClick={async () => {
            ...
            >
              {getLocale('wootzWalletButtonEnable')}
            </LeoSquaredButton>
          </OptionsRow>
        </Column>
      </CardContent>
...
export const LoadingSimulation: React.FC = () => {
  return <LoadingPanel message={getLocale('wootzWalletScanningTransaction')} />
}

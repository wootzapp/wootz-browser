...
interface Props {
  blowfishWarnings?: WootzWallet.BlowfishWarning[]
...
type Warning = Pick<WootzWallet.BlowfishWarning, 'message' | 'severity'>
  ...
  const { confirmButton, rejectButton } = React.useMemo(() => {
    return {
      confirmButton: (
        ...
        >
          {getLocale('wootzWalletAllowSpendConfirmButton')}
        </LeoSquaredButton>
      ),
      rejectButton: (
        <LeoSquaredButton
        ...
        >
          {getLocale('wootzWalletAllowSpendRejectButton')}
        </LeoSquaredButton>
      )
    }
  }, [
      ...
      {rejectAllTransactions && transactionsQueueLength > 1 && (
        <Row padding={rejectAllButtonRowPadding}>
          <QueueStepButton onClick={rejectAllTransactions}>
            {getLocale('wootzWalletQueueRejectAll').replace(
              '$1',

...
type EVMApprovalData =
  | WootzWallet.BlowfishERC20ApprovalData
  | WootzWallet.BlowfishERC721ApprovalData
  | WootzWallet.BlowfishERC721ApprovalForAllData
  | WootzWallet.BlowfishERC1155ApprovalForAllData
...
export const EvmNativeAssetOrErc20TokenTransfer = ({
  ...
  transfer:
    | WootzWallet.BlowfishERC20TransferData
    | WootzWallet.BlowfishNativeAssetTransferData
  network: ChainInfo
}): JSX.Element => {
  // queries
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {getLocale(isReceive ? 'wootzWalletReceive' : 'wootzWalletSend')}
        </Text>
...
export const NonFungibleErcTokenTransfer = ({
  ...
  network: ChainInfo
  transfer: Pick<
    | WootzWallet.BlowfishERC721TransferData
    | WootzWallet.BlowfishERC1155TransferData,
...
export const ErcTokenApproval = ({
  ...
  | {
      isERC20: true
      approval: WootzWallet.BlowfishERC20ApprovalData
      isApprovalForAll?: false
    }
)): JSX.Element => {
  /**
   * Not available for `WootzWallet.BlowfishERC1155ApprovalForAllData`
   */
  ...
  const beforeAmount = React.useMemo(() => {
    ...
    if (
      ...
      return getLocale('wootzWalletUnlimitedAssetAmount').replace(
        '$1',
        assetSymbol || ''
      )
    }
  ...
  return (
    ...
    >
      ...
      >
        <StateChangeText>
          <span>{getLocale('wootzWalletFrom')}</span>
          <strong>{beforeAmount}</strong>
          <ArrowRightIcon />
          <span>{getLocale('wootzWalletSwapTo')}</span>
      ...
      >
        ...
        >
          <Text textSize='11px'>
            {getLocale('wootzWalletSpenderAddress').replace(
              '$1',
...
function getTransferTokenVerificationStatusText(
  ...
  if (!transfer.asset.verified) {
    return getLocale('wootzWalletTokenIsUnverified')
  }
  return transfer.asset.lists.length > 1
    ? getLocale('wootzWalletTokenIsVerifiedByLists').replace(
        '$1',
        transfer.asset.lists.length.toString()
      )
    : getLocale('wootzWalletTokenIsVerified')
}

export function getComponentForEvmApproval(
  approval: WootzWallet.BlowfishEVMStateChange,
  network: ChainInfo
) {
...
export function getComponentForEvmTransfer(
  transfer: WootzWallet.BlowfishEVMStateChange,
  network: ChainInfo
) {

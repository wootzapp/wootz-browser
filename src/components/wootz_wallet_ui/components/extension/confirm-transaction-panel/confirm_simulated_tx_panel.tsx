...
type Props =
  | {
      simulationType: 'EVM'
      txSimulation: WootzWallet.EVMSimulationResponse
    }
  | {
      simulationType: 'SVM'
...
export const ConfirmSimulatedTransactionPanel = ({
  ...
  const simulationHasOutgoingSPLandSOLTransfers =
    ...
    txSimulation.expectedStateChanges.some(
      (sim) =>
        sim.rawInfo.kind ===
          WootzWallet.BlowfishSolanaRawInfoKind.kSplTransfer &&
        sim.rawInfo.data.splTransferData?.diff.sign ===
          WootzWallet.BlowfishDiffSign.kMinus
    ) &&
    txSimulation.expectedStateChanges.some(
      (sim) =>
        sim.rawInfo.kind ===
          WootzWallet.BlowfishSolanaRawInfoKind.kSolTransfer &&

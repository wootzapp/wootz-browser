...
describe('Pending Transaction Panel', () => {
  ...
  test(
    ...
    async () => {
      render(
        <WalletPanelTestWrapper
          walletApiDataOverrides={{
            simulationOptInStatus: WootzWallet.BlowfishOptInStatus.kUnset
          }}
        >
          <PendingTransactionPanel
            selectedPendingTransaction={mockTransactionInfo}
      ...
      expect(
        screen.getByText(getLocale('wootzWalletEnableTransactionSimulation'))
      ).toBeVisible()
    }
  )
  ...
  describe('EVM Simulations', () => {
    test('should show the retry button if the transaction simulation fails', async () => {
      ...
      expect(
        screen.getByText(getLocale('wootzWalletButtonRetry'))
      ).toBeVisible()
    })
    ...
    test('should show the simulated transaction panel after simulation is successful', async () => {
      ...
      expect(
        screen.queryByText(getLocale('wootzWalletButtonRetry'))
      ).not.toBeInTheDocument()

      // expected changes should be displayed
      expect(
        screen.queryByText(getLocale('wootzWalletEstimatedBalanceChange'))
      ).toBeInTheDocument()
      const approvalDetailsGrouping = screen.getByText(
        getLocale('wootzWalletApprovalDetails')
      )
  ...
  describe('SVM Simulations', () => {
    ...
    test('should show the simulated transaction panel after simulation is successful', async () => {
      ...
      expect(
        screen.queryByText(getLocale('wootzWalletAuthorityChange'))
      ).toBeInTheDocument()
      ...
      expect(
        screen.queryByText(getLocale('wootzWalletStaker'))
      ).toBeInTheDocument()
      expect(
        screen.queryByText(getLocale('wootzWalletWithdrawer'))
      ).toBeInTheDocument()
      ...
      expect(
        screen.queryByText(
          getLocale('wootzWalletSimulationWarningApprovalToEoa')
        )
      ).toBeInTheDocument()
    })
  })

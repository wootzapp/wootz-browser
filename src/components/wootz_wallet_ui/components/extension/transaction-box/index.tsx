...
const txKeys = Object.keys(WootzWallet.TransactionType)
  ...
  if (dataArray.length === 0 && !btcData && !zecData && !solData) {
    return (
      <CodeSnippet>
        <code>
          <CodeSnippetText>
            {getLocale('wootzWalletConfirmTransactionNoData')}
          </CodeSnippetText>
        </code>
      </CodeSnippet>
    )
  ...
  return (
    <>
        ...
        <>
          {!!Number(sendOptions?.maxRetries?.maxRetries) && (
            <DetailColumn key={'maxRetries'}>
              <TransactionText>
                {getLocale('wootzWalletSolanaMaxRetries')}:
              </TransactionText>
          ...
          {sendOptions?.preflightCommitment && (
            <DetailColumn key={'preflightCommitment'}>
              <TransactionText>
                {getLocale('wootzWalletSolanaPreflightCommitment')}:
              </TransactionText>
          ...
          {sendOptions?.skipPreflight && (
            <DetailColumn key={'skipPreflight'}>
              <TransactionText>
                {getLocale('wootzWalletSolanaSkipPreflight')}:
              </TransactionText>
              <DetailText>
      ...
      {solData || dataArray ? (
        <DetailColumn>
          <TransactionText>
            {getLocale('wootzWalletTransactionDetailBoxFunction')}:
          </TransactionText>

...
// Constants
import { WootzWallet, TransactionInfoLookup } from '../../../constants/types'
...
export function TransactionStatus({ transactionLookup }: Props) {
  ...
  const completePrimaryCTAText =
    transactionsQueueLength === 0
      ...
      : getLocale('wootzWalletButtonNext')
  ...
  if (
    tx.txStatus === WootzWallet.TransactionStatus.Submitted ||
    tx.txStatus === WootzWallet.TransactionStatus.Signed
  ) {
    return (
  ...
  if (tx.txStatus === WootzWallet.TransactionStatus.Confirmed) {
    return (
      <TransactionComplete
        headerTitle={transactionIntent}
        description={getLocale('wootzWalletTransactionCompleteDescription')}
  ...
  if (tx.txStatus === WootzWallet.TransactionStatus.Error) {
    ...
    const errorCode =
      ...
      WootzWallet.ProviderError.kUnknown

    const errorDetailContent =
      providerError && `${errorCode}: ${providerError.message}`
    const customDescription =
      errorCode ===
      WootzWallet.ZCashProviderError.kMultipleTransactionsNotSupported

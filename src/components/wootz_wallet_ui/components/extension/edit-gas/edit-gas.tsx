...
interface Props {
  ...
  selectedNetwork: WootzWallet.NetworkInfo
...
export const EditGas = ({
  ...
  const gasLimitComponent = React.useMemo(
    () => (
      <>
        <InputLabel>{getLocale('wootzWalletEditGasLimit')}</InputLabel>
        <Input
          placeholder='0'
          type='text'
        ...
        />
        {gasLimit === '0' && (
          <ErrorText>{getLocale('wootzWalletEditGasLimitError')}</ErrorText>
        )}
      </>
    ),
  ...
  return (
    <Panel
      ...
      title={
        isEIP1559Transaction
          ? getLocale('wootzWalletEditGasTitle1')
          : getLocale('wootzWalletEditGasTitle2')
      }
    >
      <StyledWrapper>
        {isEIP1559Transaction && (
          <Description>
            {getLocale('wootzWalletEditGasDescription')}
          </Description>
        )}
        {showCustomMaxPriorityPanel && (
          <FormColumn>
            <CurrentBaseRow>
              <CurrentBaseText>
                {getLocale('wootzWalletEditGasBaseFee')}
              </CurrentBaseText>
              <CurrentBaseText>
                {`${new Amount(baseFeePerGas).divideByDecimals(9).format()} 
                  ${getLocale('wootzWalletEditGasGwei')}`}
              </CurrentBaseText>
            </CurrentBaseRow>
            ...
            <InputLabel>
              {getLocale('wootzWalletEditGasPerTipLimit')}
            </InputLabel>
            <Input
              placeholder='0'
              type='number'
            ...
            {/* Gas-Per-Price Limit */}
            <InputLabel>
              {getLocale('wootzWalletEditGasPerPriceLimit')}
            </InputLabel>
            <Input
              placeholder='0'
              min={0}
            ...
            <MaximumFeeRow>
              <MaximumFeeText>
                {getLocale('wootzWalletEditGasMaximumFee')}
              </MaximumFeeText>
              <MaximumFeeText>
        ...
        {showSuggestedMaxPriorityPanel && (
          <SliderWrapper>
            <SliderLabel>{getLocale('wootzWalletEditGasMaxFee')}:</SliderLabel>
            <SliderValue>
            ...
            />
            <SliderLabelRow>
              <SliderLabel>{getLocale('wootzWalletEditGasLow')}</SliderLabel>
              <SliderLabel>
                {getLocale('wootzWalletEditGasOptimal')}
              </SliderLabel>
              <SliderLabel>{getLocale('wootzWalletEditGasHigh')}</SliderLabel>
            </SliderLabelRow>
          </SliderWrapper>
        )}
        ...
        {!isEIP1559Transaction && (
          <FormColumn>
            ...
            <InputLabel>
              {getLocale('wootzWalletEditGasPerGasPrice')}
            </InputLabel>
            <Input
              placeholder='0'
              type='number'
            ...
            {isZeroGasPrice && (
              <WarningText>
                {getLocale('wootzWalletEditGasZeroGasPriceWarning')}
              </WarningText>
            )}
          </FormColumn>
        )}
        ...
        <ButtonRow>
          <NavButton
            ...
            text={
              !isEIP1559Transaction
                ...
                ? getLocale('wootzWalletEditGasSetSuggested')
                : getLocale('wootzWalletEditGasSetCustom')
            }
            onSubmit={
              !isEIP1559Transaction
                ? onCancel
          ...
          <NavButton
            buttonType='primary'
            text={getLocale('wootzWalletAccountSettingsSave')}
            onSubmit={onSave}

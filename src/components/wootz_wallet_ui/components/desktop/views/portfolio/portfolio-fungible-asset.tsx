...
export const PortfolioFungibleAsset = () => {
  ...
  const fullAssetBalance = React.useMemo(() => {
    const amounts = candidateAccounts.map((account: WootzWallet.AccountInfo) =>
      getBalance(
        account.accountId,
        selectedAssetFromParams,
  ...
  const isSelectedAssetBridgeSupported = React.useMemo(() => {
    ...
    return (
      ...
      selectedAssetFromParams.chainId === WootzWallet.MAINNET_CHAIN_ID
    )
  }, [selectedAssetFromParams])
            ...
            {isSelectedAssetBridgeSupported && (
              <div>
                <LeoSquaredButton onClick={onBridgeToAuroraButton}>
                  {getLocale('wootzWalletBridgeToAuroraButton')}
                </LeoSquaredButton>
              </div>
            )}
          </ButtonRow>

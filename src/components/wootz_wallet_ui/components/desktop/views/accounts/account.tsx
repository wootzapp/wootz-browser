...
const INDIVIDUAL_TESTNET_ACCOUNT_KEYRING_IDS = [
  ...
  WootzWallet.KeyringId.kZCashTestnet
]
...
const coinSupportsNFTs = (coin: WootzWallet.CoinType) => {
  return [WootzWallet.CoinType.ETH, WootzWallet.CoinType.SOL].includes(coin)
}

const coinSupportsAssets = (coin: WootzWallet.CoinType) => {
  return [WootzWallet.CoinType.ETH, WootzWallet.CoinType.SOL].includes(coin)
}
...
export const Account = () => {
  ...
  const accountsTokensList = React.useMemo(() => {
    ...
    const hasLocalHostNetwork = networkList.some(
      (network) =>
        network.chainId === WootzWallet.LOCALHOST_CHAIN_ID &&
    ...
    const localHostCoins = userVisibleTokensInfo.filter(
      (token) => token.chainId === WootzWallet.LOCALHOST_CHAIN_ID
    )
    ...
    if (
      accountsLocalHost &&
      hasLocalHostNetwork &&
      selectedAccount.accountId.keyringId !== WootzWallet.KeyringId.kFilecoin
    ) {
  ...
  const onClickMenuOption = React.useCallback(
    (option: AccountModalTypes) => {
      ...
      if (
        ...
        (selectedAccount?.accountId.coin === WootzWallet.CoinType.ETH ||
          selectedAccount?.accountId.coin === WootzWallet.CoinType.SOL)
      ) {
      ...
      {selectedTab === AccountPageTabs.AccountNFTsSub &&
        (nonFungibleTokens?.length !== 0 ? (
          <NFTsWrapper fullWidth={true}>
            <NftGrid>
              {nonFungibleTokens?.map((nft: WootzWallet.BlockchainToken) => (
                <NFTGridViewItem

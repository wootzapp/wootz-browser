...
export const AutoDiscoveryEmptyState = ({
  ...
  const { duringTag: refreshBtnText, afterTag } = splitStringForTag(
    getLocale('wootzWalletAutoDiscoveryEmptyStateActions')
  )
    ...
    <StyledWrapper>
      {isRefreshingTokens ? (
        <>
          ...
          <RefreshText>
            {getLocale('wootzWalletAutoDiscoveryEmptyStateRefresh')}...
          </RefreshText>
        </>
      ) : (
        <>
          <Heading>
            {getLocale('wootzWalletAutoDiscoveryEmptyStateHeading')}
          </Heading>
          <Description>
            {getLocale('wootzWalletAutoDiscoveryEmptyStateSubHeading')}
          </Description>
          <Row
            margin='48px 0 8px 0'
            marginBottom={8}
          >
            <Description>
              {getLocale('wootzWalletAutoDiscoveryEmptyStateFooter')}
            </Description>
          </Row>
          <Description>

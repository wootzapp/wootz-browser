...
export const ExploreWeb3View = () => {
  ...
  const [dappCategories, categoryDappsMap, visibleCategories] =
    React.useMemo(() => {
      if (!topDappsForChains) {
        return [[], new Map<string, WootzWallet.Dapp[]>(), []]
      }

      const categoriesSet = new Set<string>()
      const categoriesMap = new Map<string, WootzWallet.Dapp[]>()
  ...
  const visibleDappsMap = React.useMemo(() => {
    ...
    const filterResultsMap = new Map<string, WootzWallet.Dapp[]>()
    ...
    <>
      ...
      >
        ...
        {showFiltersRow && (
          ...
          >
            ...
            <PlainButton onClick={onClearFilters}>
              {getLocale('wootzWalletClearFilters')}
            </PlainButton>
          </Row>
        )}
        ...
        selectedCategoryDapps?.length === 0 ? (
          ...
          <Row>
            <h2>{getLocale('wootzWalletNoDappsFound')}</h2>
          </Row>
        )}
      </Column>

...
import { DialogProps } from '@wootz/leo/react/dialog'
...
import type { WootzWallet } from '../../../../constants/types'
...
export const DappDetails = ({ dapp, ...rest }: DappDetailsProps) => {
  ...
  return (
    <DappDetailDialog
      ...
      showClose={true}
    >
      <Title slot='title'>{getLocale('wootzWalletDetails')}</Title>
      <Column
        fullHeight
        fullWidth
      ...
      >
        ...
        >
          <Row gap={'8px'}>
            <DappMetric
              metricName={getLocale('wootzWalletActiveWallets')}
              meticValue={dapp.uaw}
              isUSDValue={false}
            />
            <DappMetric
              metricName={getLocale('wootzWalletTransactions')}
          ...
          <Row gap={'8px'}>
            <DappMetric
              metricName={getLocale('wootzWalletMarketDataVolumeColumn')}
              meticValue={dapp.volume}
              isUSDValue={true}
            />
            <DappMetric
              metricName={getLocale('wootzWalletBalance')}
        ...
        >
          <Text textAlign='left'>{getLocale('wootzWalletNetworks')}</Text>
          <Row
            justifyContent={'flex-start'}
            alignItems={'flex-start'}
          >
        ...
        {isHttpsUrl(dapp.website) ? (
          <ExternalLink
            href={dapp.website}
            text={getLocale('wootzWalletVisitDapp').replace('$1', dapp.name)}
            width='100%'
          />
        ) : null}
      </Column>

...
const getErrorMessage = (
  ...
  if (typeof error === 'undefined') {
    return { error: getLocale('wootzWalletUnknownInternalError'), userHint: '' }
  }
  ...
  if (
    ...
    // Unknown Error
    return {
      error: getLocale('wootzWalletConnectHardwareInfo2').replace(
        '$1',
        accountTypeName
      ),
      userHint: ''
  ...
  if (
    ...
    return {
      error: error.message,
      userHint: getLocale('wootzWalletConnectHardwareWrongApplicationUserHint')
    }
  }
...
export const HardwareWalletConnect = ({
  ...
  const [selectedDerivationScheme, setSelectedDerivationScheme] =
    React.useState<HardwareDerivationScheme>(
      selectedAccountType.coin === WootzWallet.CoinType.SOL
  ...
  const onFilecoinNetworkChanged = React.useCallback(
    (network: FilecoinNetwork) => {
      ...
      onConnectHardwareWallet({
        hardware: WootzWallet.LEDGER_HARDWARE_VENDOR,
        startIndex: 0,
  ...
  const onAddHardwareAccounts = React.useCallback(
    async (accounts: WootzWallet.HardwareWalletAccount[]) => {
      try {
  ...
  const onSelectLedger = React.useCallback(() => {
    if (selectedHardwareWallet !== WootzWallet.LEDGER_HARDWARE_VENDOR) {
      setConnectionError(undefined)
    }

    selectVendor(WootzWallet.LEDGER_HARDWARE_VENDOR)
  }, [selectedHardwareWallet, selectVendor])

  const onSelectTrezor = React.useCallback(() => {
    if (selectedHardwareWallet !== WootzWallet.TREZOR_HARDWARE_VENDOR) {
      setConnectionError(undefined)
    }

    selectVendor(WootzWallet.TREZOR_HARDWARE_VENDOR)
  }, [selectedHardwareWallet, selectVendor])
  ...
  const preAddedHardwareWalletAccounts = React.useMemo(() => {
    return savedAccounts.filter(
      (account) => account.accountId.kind === WootzWallet.AccountKind.kHardware
    )
  }, [savedAccounts])
  ...
  if (!selectedHardwareWallet) {
    return (
      <>
        <HardwareButton
          title={getLocale('wootzWalletConnectHardwareLedger')}
          description={getLocale(
            'wootzWalletConnectHardwareDeviceDescription'
          ).replace('$1', getLocale('wootzWalletConnectHardwareLedger'))}
          onClick={onSelectLedger}
        />
        <Divider />
        <HardwareButton
          title={getLocale('wootzWalletConnectHardwareTrezor')}
          description={getLocale(
            'wootzWalletConnectHardwareDeviceDescription'
          ).replace('$1', getLocale('wootzWalletConnectHardwareTrezor'))}
    ...
    <Column>
      ...
      <Instructions mode={connectionError ? 'error' : 'info'}>
        {connectionError
          ...
          : getLocale('wootzWalletConnectHardwareAuthorizationNeeded').replace(
              '$1',
              selectedHardwareWallet
            )}
      </Instructions>
      ...
      {showAuthorizeDevice ? (
        ...
        >
          <div slot='loading'>
            {getLocale('wootzWalletConnectingHardwareWallet')}
          </div>
          {!isConnecting && getLocale('wootzWalletAddAccountConnect')}
        </ContinueButton>
      )}
    </Column>
  )

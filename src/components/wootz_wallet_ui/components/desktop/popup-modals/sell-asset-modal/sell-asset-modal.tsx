...
export const SellAssetModal = (props: Props) => {
  ...
  return (
    <PopupModal
      title={`${getLocale('wootzWalletSell')} ${selectedAsset.name}`}
    ...
    >
      <StyledWrapper>
          ...
          {insufficientBalance && !isInvalidAmount && (
            <ErrorBox>
              ...
              <Text
                ...
                textAlign='left'
              >
                {getLocale('wootzWalletNotEnoughBalance').replace(
                  '$1',
                  selectedAsset.symbol
                )}
              </Text>
        ...
        >
          {getLocale('wootzWalletSellWithProvider').replace('$1', 'Ramp')}
        </LeoSquaredButton>
      </StyledWrapper>
    </PopupModal>
  )

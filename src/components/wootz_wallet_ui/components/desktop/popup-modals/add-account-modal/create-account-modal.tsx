...
export const CreateAccountModal = () => {
  ...
  const suggestedAccountName = React.useMemo(() => {
    ...
    } ${getLocale('wootzWalletSubviewAccount')} ${
      //
      accountTypeLength
    }`
  ...
  const targetKeyringId = React.useMemo(() => {
    ...
    const network =
      ...
      (selectedAccountType.coin === WootzWallet.CoinType.BTC &&
        bitcoinNetwork) ||
      (selectedAccountType.coin === WootzWallet.CoinType.ZEC && zcashNetwork) ||
      undefined
  ...
  const modalTitle = selectedAccountType
    ? getLocale('wootzWalletCreateAccount').replace(
        '$1',
        selectedAccountType.name
      )
    : getLocale('wootzWalletCreateAccountButton')
    ...
    >
      ...
      {selectedAccountType && (
        <CreateAccountStyledWrapper>
          ...
          {selectedAccountType?.coin === WootzWallet.CoinType.ZEC && (
            <Dropdown
              value={zcashNetwork}
      ...
      {!selectedAccountType && (
        <SelectAccountType
          ...
          buttonText={getLocale('wootzWalletAddAccountCreate')}

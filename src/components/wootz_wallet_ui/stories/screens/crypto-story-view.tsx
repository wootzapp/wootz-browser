...
const CryptoStoryView = (props: Props) => {
  ...
  const [filteredAppsList, setFilteredAppsList] = React.useState<
    AppsListType[]
  >(AppsList())
  const [favoriteApps, setFavoriteApps] = React.useState<WootzWallet.AppItem[]>(
  ...
  const addToFavorites = (app: WootzWallet.AppItem) => {
    const newList = [...favoriteApps, app]
    setFavoriteApps(newList)
  }
  const removeFromFavorites = (app: WootzWallet.AppItem) => {
  ...
  const onClickSettings = () => {
    ...
    alert('Will Nav to wootz://settings/wallet')
  }
        ...
        {!hideNav && (
          <>
            ...
            {showDefaultWalletBanner && (
              <WalletBanner
                description={getLocale('wootzWalletDefaultWalletBanner')}
                onDismiss={onDismissDefaultWalletBanner}
                onClick={onClickSettings}
                bannerType='warning'
                buttonText={getLocale('wootzWalletWalletPopupSettings')}
              />
            )}

            {needsBackup && showBackupWarning && (
              <WalletBanner
                description={getLocale('wootzWalletBackupWarningText')}
                onDismiss={onDismissBackupWarning}
                onClick={onShowBackup}
                bannerType='danger'
                buttonText={getLocale('wootzWalletBackupButton')}
              />
            )}
          </>
        )}

...
// Types
import { WootzWallet, externalWalletProviders } from '../constants/types'
...
export const getRewardsProviderName = (provider?: string) => {
  ...
  const localeString = `wootzWallet${capitalized}`
  try {
...
export const getRewardsAccountName = (provider?: string) => {
  ...
  return getLocale('wootzWalletRewardsAccount').replace(
    '$1',
...
export const getRewardsTokenDescription = (
  ...
  return getLocale('wootzWalletWootzRewardsDescription').replace(
    '$1',
    getRewardsProviderName(provider)
  )
}
...
export const getIsRewardsNetwork = (
  network?: Pick<WootzWallet.NetworkInfo, 'chainId'>
) => {
  if (!network) {
    return false
  }
...
export const getIsRewardsToken = (
  token?: Pick<WootzWallet.BlockchainToken, 'chainId'>
) => {
  if (!token) {
    return false
  }

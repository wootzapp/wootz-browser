...
export function isValidFilAddress(value: string): boolean {
  if (
    !value.startsWith(WootzWallet.FILECOIN_MAINNET) &&
    !value.startsWith(WootzWallet.FILECOIN_TESTNET)
  ) {
    return false
  }
...
export const suggestNewAccountName = (
  ...
  network: Pick<WootzWallet.NetworkInfo, 'coin' | 'symbolName' | 'chainId'>
) => {
  ...
  return `${network.symbolName} ${getLocale(
    SupportedTestNetworks.includes(network.chainId)
      ? 'wootzWalletTestNetworkAccount'
      : 'wootzWalletSubviewAccount'
  )} ${accountTypeLength}`
}

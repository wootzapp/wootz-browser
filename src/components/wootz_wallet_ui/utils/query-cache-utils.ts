...
export const TX_CACHE_TAGS = {
  ID: (txId: WootzWallet.TransactionInfo['id']) => ({
    type: 'Transactions' as const,
    id: txId
  }),
  IDS: (txIds: Array<WootzWallet.TransactionInfo['id']>) =>
    txIds.map((id) => TX_CACHE_TAGS.ID(id)),
  FOR_COIN_TYPE: (coinType: WootzWallet.CoinType) =>
    ({
      type: 'Transactions',
  ...
  FROM_ACCOUNT_ID: (fromAccountId: WootzWallet.AccountId) =>
    ({
      type: 'Transactions',
  ...
  LISTS: ({
    ...
    chainId: string | null
    coin: WootzWallet.CoinType | null
    fromAccountId: WootzWallet.AccountId | null
  }) =>
    [

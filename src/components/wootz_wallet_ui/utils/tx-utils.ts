...
export type FileCoinTransactionInfo = TransactionInfo & {
  txDataUnion: {
    filTxData: WootzWallet.FilTxData
...
export interface ParsedTransaction
  extends ParsedTransactionFees,
    Pick<WootzWallet.TransactionInfo, 'txType'> {
  // Common fields
  id: string
  hash: string
  nonce: string
  createdTime: SerializableTimeDelta
  status: WootzWallet.TransactionStatus
  senderLabel: string
  recipient: string
  ...
  intent: string
  chainId: string
  originInfo?: WootzWallet.OriginInfo | undefined
  ...
  erc721BlockchainToken?: WootzWallet.BlockchainToken
  ...
  // Swap
  sellToken?: WootzWallet.BlockchainToken
  sellAmount?: Amount
  sellAmountWei?: Amount
  buyToken?: WootzWallet.BlockchainToken
  minBuyAmount?: Amount
...
export const getLocaleKeyForTxStatus = (
  ...
  switch (status) {
    case WootzWallet.TransactionStatus.Unapproved:
      return 'wootzWalletTransactionStatusUnapproved'
    case WootzWallet.TransactionStatus.Approved:
      return 'wootzWalletTransactionStatusApproved'
    case WootzWallet.TransactionStatus.Rejected:
      return 'wootzWalletTransactionStatusRejected'
    case WootzWallet.TransactionStatus.Submitted:
      return 'wootzWalletTransactionStatusSubmitted'
    case WootzWallet.TransactionStatus.Confirmed:
      return 'wootzWalletTransactionStatusConfirmed'
    case WootzWallet.TransactionStatus.Error:
      return 'wootzWalletTransactionStatusError'
    case WootzWallet.TransactionStatus.Dropped:
      return 'wootzWalletTransactionStatusDropped'
    case WootzWallet.TransactionStatus.Signed:
      return 'wootzWalletTransactionStatusSigned'
    default:
      return ''
  }
}
...
export function isSolanaTransaction(
  ...
  return (
    ...
    (txType === WootzWallet.TransactionType.Other && solanaTxData !== undefined)
  )
}
...
export function isSolanaDappTransaction(
  ...
  return (
    ...
    [
      ...
      WootzWallet.TransactionType.Other
    ].includes(tx.txType)
  )
}
...
export function isSolanaSplTransaction(
  ...
  return (
    tx.txType === WootzWallet.TransactionType.SolanaSPLTokenTransfer ||
    tx.txType ===
...
>(
  ...
  // Native Asset Send
  if (
    tx.txType === WootzWallet.TransactionType.SolanaSystemTransfer ||
    tx.txType === WootzWallet.TransactionType.ETHSend ||
...
export const getETHSwapTransactionBuyAndSellTokens = ({
  ...
  tx: TransactionInfo | undefined
  nativeAsset?: WootzWallet.BlockchainToken
  tokensList: WootzWallet.BlockchainToken[]
}): {
  buyToken?: WootzWallet.BlockchainToken
  sellToken?: WootzWallet.BlockchainToken
  buyAmount: Amount
  ...
  if (
    !tx ||
    !tx.swapInfo ||
    tx.txType !== WootzWallet.TransactionType.ETHSwap
  ) {
    return {
      buyToken: undefined,
      sellToken: undefined,
    ...
    tx.swapInfo.fromAsset === NATIVE_EVM_ASSET_CONTRACT_ADDRESS
      ...
      : findTokenByContractAddress(tx.swapInfo.fromAsset, tokensList) ||
        ...
        ({
          ...
          isSpam: false,
          visible: true
        } as WootzWallet.BlockchainToken)
...
interface GetTransactionTransferredValueArgs {
  ...
  token?: WootzWallet.BlockchainToken
  sellToken?: WootzWallet.BlockchainToken
  txAccount: WootzWallet.AccountInfo | undefined
  txNetwork: WootzWallet.NetworkInfo | undefined
}
...
export function getTransactionTransferredValue(
  ...
  // ERC20 Transfers
  if (
    tx.txType === WootzWallet.TransactionType.ERC20Approve ||
    tx.txType === WootzWallet.TransactionType.ERC20Transfer
  ) {
  ...
  // ERC721
  if (
    tx.txType === WootzWallet.TransactionType.ERC721TransferFrom ||
    tx.txType === WootzWallet.TransactionType.ERC721SafeTransferFrom
  ) {
  ...
  // ETH Swap
  if (tx.txType === WootzWallet.TransactionType.ETHSwap) {
...
export const getTransactionApprovalTargetAddress = (
  ...
  if (tx.txType === WootzWallet.TransactionType.ERC20Approve) {
...
export function getTransactionDecimals({
  ...
  sellToken?: WootzWallet.BlockchainToken
  erc721Token?: WootzWallet.BlockchainToken
  token?: WootzWallet.BlockchainToken
}) {
  if (sellToken) {
...
export const getTransactionErc721TokenId = (
  ...
  if (
    [
      WootzWallet.TransactionType.ERC721TransferFrom,
      WootzWallet.TransactionType.ERC721SafeTransferFrom
    ].includes(tx.txType)
  ) {
...
export const isSendingToKnownTokenContractAddress = (
  ...
  // ERC20Transfer
  if (tx.txType === WootzWallet.TransactionType.ERC20Transfer) {
...
export const transactionHasSameAddressError = (
  ...
  if (txType === WootzWallet.TransactionType.ERC20Transfer) {
  ...
  if (
    txType === WootzWallet.TransactionType.ERC721TransferFrom ||
    txType === WootzWallet.TransactionType.ERC721SafeTransferFrom
  ) {
    // The owner of the ERC721 must not be confused with the caller
    // (fromAddress).
    // (address owner, address to, uint256 tokenId)
  ...
  if (txType === WootzWallet.TransactionType.ERC20Approve) {
  ...
  if (
    WootzWallet.TransactionType.ETHSend ||
    WootzWallet.TransactionType.ETHSwap ||
    WootzWallet.TransactionType.Other
  ) {
    return false
  }
...
export function getGasFeeFiatValue({
  ...
  txNetwork?: Pick<WootzWallet.NetworkInfo, 'decimals'>
}) {
...
export const accountHasInsufficientFundsForTransaction = ({
  ...
  // Eth Swap
  if (txType === WootzWallet.TransactionType.ETHSwap) {
...
export function getTransactionTransferredToken({
  ...
  tx: TransactionInfo
  txNetwork?: WootzWallet.NetworkInfo
  token?: WootzWallet.BlockchainToken
  sellToken?: WootzWallet.BlockchainToken
}): WootzWallet.BlockchainToken | undefined {
  if (tx.txType === WootzWallet.TransactionType.ETHSwap) {
    return sellToken
  }
...
export function getTransactionTokenSymbol({
  ...
  tx: TransactionInfo
  txNetwork?: Pick<WootzWallet.NetworkInfo, 'symbol'>
  token?: WootzWallet.BlockchainToken
  sellToken?: WootzWallet.BlockchainToken
}): string {
  if (isSolanaDappTransaction(tx)) {
...
export const getTransactionIntent = ({
  ...
  sellToken?: WootzWallet.BlockchainToken
  token?: WootzWallet.BlockchainToken
  transactionNetwork?: WootzWallet.NetworkInfo
  tx: TransactionInfo
}): string => {
  // ERC20 Approve
  if (tx.txType === WootzWallet.TransactionType.ERC20Approve) {
    return (
      toProperCase(getLocale('wootzWalletApprovalTransactionIntent')) +
        ' ' +
        token?.symbol ?? ''
    )
  }
  ...
  // ERC20 Transfer
  if (tx.txType === WootzWallet.TransactionType.ERC20Transfer) {
    return getLocale('wootzWalletTransactionIntentSend').replace(
      '$1',
  ...
  // Solana Dapps
  if (isSolanaDappTransaction(tx)) {
    return tx.txType === WootzWallet.TransactionType.SolanaSwap
      ? getLocale('wootzWalletSwap')
      : getLocale('wootzWalletTransactionIntentDappInteraction')
  }
  ...
  // ETHSwap
  if (tx.txType === WootzWallet.TransactionType.ETHSwap) {
    return getLocale('wootzWalletTransactionIntentSwap')
...
export const parseTransactionWithoutPrices = ({
  ...
  tx: TransactionInfo
  transactionAccount: WootzWallet.AccountInfo
  transactionNetwork: WootzWallet.NetworkInfo
  tokensList: WootzWallet.BlockchainToken[]
  ...
  const missingGasLimitError = isMissingGasLimit
    ? getLocale('wootzWalletMissingGasLimitError')
    : undefined
  ...
  const contractAddressError = isSendingToKnownTokenContractAddress(
    ...
    ? getLocale('wootzWalletContractAddressError')
    : undefined
  ...
  const sameAddressError = transactionHasSameAddressError(tx)
    ? getLocale('wootzWalletSameAddressError')
    : undefined
...
export function toTxDataUnion<D extends keyof WootzWallet.TxDataUnion>(
  unionItem: Pick<WootzWallet.TxDataUnion, D>
) {
  return Object.assign({}, unionItem) as WootzWallet.TxDataUnion
}
...
): request is WootzWallet.SignTransactionRequest {
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
  return !!(request as WootzWallet.SignTransactionRequest | undefined)?.txData
    ?.solanaTxData
}
...
export function isSolanaSignAllTransactionsRequest(
  request?:
    | WootzWallet.SignTransactionRequest
    | WootzWallet.SignAllTransactionsRequest
): request is WootzWallet.SignAllTransactionsRequest {
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
  return !!(request as WootzWallet.SignAllTransactionsRequest | undefined)
    ?.txDatas
}

export function getTxDataFromSolSignTxRequest(
  selectedQueueData: WootzWallet.SignTransactionRequest
): WootzWallet.SolanaTxData | undefined {
  return selectedQueueData.txData?.solanaTxData
}

export function getTxDatasFromSolSignAllTxsRequest(
  selectedQueueData: WootzWallet.SignAllTransactionsRequest
): WootzWallet.SolanaTxData[] {
  return selectedQueueData.txDatas
    .map(({ solanaTxData }) => solanaTxData)
    .filter((data): data is WootzWallet.SolanaTxData => !!data)
}
...
export const isAssociatedTokenAccountCreationTx = (
  tx: Pick<WootzWallet.TransactionInfo, 'txType'> | undefined
) =>
  tx?.txType ===
  WootzWallet.TransactionType
    .SolanaSPLTokenTransferWithAssociatedTokenAccountCreation

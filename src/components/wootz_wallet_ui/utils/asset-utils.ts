...
export const getUniqueAssets = (assets: WootzWallet.BlockchainToken[]) => {
...
export const isSelectedAssetInAssetOptions = (
  ...
  assetOptions: WootzWallet.BlockchainToken[]
) => {
  return (
...
export const getRampAssetSymbol = (
  ...
  if (
    ...
    asset.chainId === WootzWallet.MAINNET_CHAIN_ID
  ) {
  ...
  if (
    asset.chainId === WootzWallet.AVALANCHE_MAINNET_CHAIN_ID &&
...
export const isBat = ({
  symbol
}: Pick<WootzWallet.BlockchainToken, 'symbol'>) => {
...
export const findTokenByContractAddress = <
  T extends Pick<WootzWallet.BlockchainToken, 'contractAddress'>
>(
...
export const isNativeAsset = (
  token: Pick<WootzWallet.BlockchainToken, 'contractAddress'>
) => token.contractAddress === ''

export const formatTokenBalance = (
  tokenBalanceString: string | undefined,
  selectedAsset:
    | Pick<WootzWallet.BlockchainToken, 'decimals' | 'symbol'>
    | undefined,
...
export const checkIfTokensMatch = (
  tokenOne: WootzWallet.BlockchainToken,
  tokenTwo: WootzWallet.BlockchainToken
): boolean => {
  return (
...
export function filterTokensByNetworks(
  assets: WootzWallet.BlockchainToken[],
...
export const checkIfTokenNeedsNetworkIcon = (
  network: Pick<WootzWallet.NetworkInfo, 'chainId' | 'symbol'>,
  contractAddress: string
) => {
  return (
...
export function makeCountCollectionAssetsInRegistry(
  ...
  total: number,
  currentCollectionToken: WootzWallet.BlockchainToken,
  currentIndex: number,
  array: WootzWallet.BlockchainToken[]
) => number {
...
export function groupSpamAndNonSpamNfts(nfts: WootzWallet.BlockchainToken[]) {
  const results: {
    visibleUserNonSpamNfts: WootzWallet.BlockchainToken[]
    visibleUserMarkedSpamNfts: WootzWallet.BlockchainToken[]
  } = {
...
export function getAllSpamNftsAndIds(
  ...
  simpleHashSpamNfts: WootzWallet.BlockchainToken[],
  visibleUserMarkedSpamNfts: WootzWallet.BlockchainToken[]
) {
...
export const compareTokensByName = (
  a: Pick<WootzWallet.BlockchainToken, 'name'>,
  b: Pick<WootzWallet.BlockchainToken, 'name'>
) => a.name.localeCompare(b.name)

export function isTokenWatchOnly(
  token: WootzWallet.BlockchainToken,
  allAccounts: WootzWallet.AccountInfo[],
...
export function getTokensWithBalanceForAccounts(
  ...
  filteredAccounts: WootzWallet.AccountInfo[],
  allAccounts: WootzWallet.AccountInfo[],
  tokenBalancesRegistry: TokenBalancesRegistry | null | undefined,
  spamTokenBalancesRegistry: TokenBalancesRegistry | null | undefined,
  hideUnowned?: boolean
...
export const searchNfts = (
  searchValue: string,
  items: WootzWallet.BlockchainToken[]
) => {
  if (searchValue === '') {
    return items
  }
...
export const searchNftCollectionsAndGetTotalNftsFound = (
  searchValue: string,
  collections: WootzWallet.BlockchainToken[],
  collectionAssetsRegistry: Record<
    string, // collection name
    WootzWallet.BlockchainToken[]
  >
): {
  foundCollections: WootzWallet.BlockchainToken[]
...
export function getCoinTypeName(coin: WootzWallet.CoinType) {
  switch (coin) {

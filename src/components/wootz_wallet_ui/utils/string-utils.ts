...
import { WootzWallet, WalletRoutes, TokenStandards } from '../constants/types'
...
export function getIsWootzWalletOrigin({
  originSpec
}: Pick<WootzWallet.OriginInfo, 'originSpec'>) {
  try {
    const url = new URL(originSpec)
    return (
      (url.protocol === 'chrome:' || url.protocol === 'wootz:') &&
      url.host === 'wallet'
    )
  } catch (error) {
    console.log(error)
...
export const getRampNetworkPrefix = (chainId: string, isOfframp?: boolean) => {
  switch (chainId) {
    ...
    case WootzWallet.AVALANCHE_MAINNET_CHAIN_ID:
      return 'AVAXC'
    case WootzWallet.BNB_SMART_CHAIN_MAINNET_CHAIN_ID:
      return 'BSC'
    case WootzWallet.POLYGON_MAINNET_CHAIN_ID:
      return 'MATIC'
    case WootzWallet.SOLANA_MAINNET:
      return 'SOLANA'
    case WootzWallet.OPTIMISM_MAINNET_CHAIN_ID:
      return 'OPTIMISM'
    // Offramp uses CELO prefix
    case WootzWallet.CELO_MAINNET_CHAIN_ID:
      return isOfframp ? 'CELO' : ''
    case WootzWallet.FANTOM_MAINNET_CHAIN_ID:
      return 'FANTOM'
    case WootzWallet.FILECOIN_MAINNET:
      return 'FILECOIN'
    case WootzWallet.BITCOIN_MAINNET:
...
export const getWalletLocationTitle = (location: string) => {
  ...
  if (location.includes(WalletRoutes.FundWalletPageStart)) {
    return getLocale('wootzWalletBuyCryptoButton')
  }
  /** Deposit crypto */
  if (location.includes(WalletRoutes.DepositFundsPageStart)) {
    return getLocale('wootzWalletDepositFundsTitle')
  }
  /** Swap */
  if (location === WalletRoutes.Swap) {
    return getLocale('wootzWalletSwap')
  }
  if (location === WalletRoutes.Send) {
    return getLocale('wootzWalletSend')
  }
  /** Wallet */
  return getLocale('wootzWalletTitle')
}
...
export const getNFTTokenStandard = (token: WootzWallet.BlockchainToken) => {
  if (token.isNft && token.coin === WootzWallet.CoinType.SOL) {
    return TokenStandards.SPL
  }
  if (token.isErc721 && token.coin === WootzWallet.CoinType.ETH) {
...
export const isComponentInStorybook = () => {
  const nftDisplayOrigin =
    loadTimeData.getString('wootzWalletNftBridgeUrl') || ''
  return (
    nftDisplayOrigin === 'wootzWalletNftBridgeUrl' || nftDisplayOrigin === ''
  )
}

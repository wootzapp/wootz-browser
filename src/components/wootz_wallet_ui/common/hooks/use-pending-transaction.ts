...
// Constants
import { WootzWallet, emptyProviderErrorCodeUnion } from '../../constants/types'
import {
...
export const usePendingTransactions = () => {
  ...
  const tokenPriceIds = React.useMemo(
    () =>
      [txToken, networkAsset]
        .filter((t): t is WootzWallet.BlockchainToken => Boolean(t))
  ...
  const {
    ...
    transactionsNetwork &&
      transactionsNetwork.coin === WootzWallet.CoinType.ETH &&
      transactionInfo &&
  ...
  const gasFee = React.useMemo(() => {
    ...
    return txCoinType === WootzWallet.CoinType.SOL
      ? solFeeEstimate ?? ''
  ...
  const isERC20Approve =
    transactionInfo?.txType === WootzWallet.TransactionType.ERC20Approve
  ...
  const isERC721TransferFrom =
    transactionInfo?.txType === WootzWallet.TransactionType.ERC721TransferFrom
        ...
        UIActions.setTransactionProviderError({
          providerError: {
            code: {
              ...
              providerError: WootzWallet.ProviderError.kUnknown
            },
  ...
  const transactionTitle = React.useMemo(
    (): string =>
      isSolanaDappTransaction
        ? getLocale('wootzWalletApproveTransaction')
        : transactionDetails?.isSwap
        ? getLocale('wootzWalletSwap')
        : getLocale('wootzWalletSend'),
  ...
  const isLoadingGasFee = React.useMemo(() => {
    ...
    if (
      txCoinType === WootzWallet.CoinType.BTC ||
      txCoinType === WootzWallet.CoinType.ZEC
    ) {
      return false
    }

    // SOL
    if (txCoinType === WootzWallet.CoinType.SOL) {
      return isLoadingSolFeeEstimates
    }

    // FIL has gas info provided by txDataUnion
    if (txCoinType === WootzWallet.CoinType.FIL) {
      return gasFee === ''
    }

    // EVM
    if (txCoinType === WootzWallet.CoinType.ETH) {

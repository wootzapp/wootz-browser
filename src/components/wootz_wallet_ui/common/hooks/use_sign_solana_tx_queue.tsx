...
export const useProcessSignSolanaTransaction = (
  ...
  const sign = React.useCallback(async () => {
    ...
    const isHwAccount =
      props.account.accountId.kind === WootzWallet.AccountKind.kHardware

    if (props.signMode === 'signTx') {
      if (isHwAccount) {
        await signTransactionHardware({
          account: props.account,
          request: props.request as WootzWallet.SignTransactionRequest
        }).unwrap()
        return
      }
    ...
    if (props.signMode === 'signAllTxs') {
      if (isHwAccount) {
        await signAllTransactionsHardware({
          account: props.account,
          request: props.request as WootzWallet.SignAllTransactionsRequest
        }).unwrap()
        return
      }

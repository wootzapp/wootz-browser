...
const getMockedLedgerEthKeyring = (
  expectedPath: string,
  expectedData: string | WootzWallet.TransactionInfo,
  signed?: SignHardwareOperationResult
) => {
  return {
    type: (): HardwareVendor => {
      return WootzWallet.LEDGER_HARDWARE_VENDOR
    },
...
const getMockedLedgerFilKeyring = (
  expectedMessage: string | WootzWallet.TransactionInfo,
  signed?: SignHardwareOperationResult
) => {
  return {
    type: (): HardwareVendor => {
...
const getMockedTrezorKeyring = (
  ...
  return {
    type: (): HardwareVendor => {
      return WootzWallet.TREZOR_HARDWARE_VENDOR
    },
    signTransaction: async (
      path: string,
      data: string
...
test('Test sign Ledger transaction, approved, no message to sign', () => {
  ...
  return expect(
    ...
    hardwareTransactionErrorResponse('wootzWalletNoMessageToSignError')
  )
})

test('Test sign Ledger transaction, approved, device error', () => {
  return expect(signEthTransactionWithLedger()).resolves.toStrictEqual(
    hardwareTransactionErrorResponseWithCode('wootzWalletSignOnDeviceError')
  )
})
...
test('Test sign Ledger transaction, approved, processing error', () => {
  return expect(
    ...
    hardwareTransactionErrorResponse('wootzWalletProcessTransactionError')
  )
})
...
test('Test sign Trezor transaction, approve failed', () => {
  ...
  return expect(
    ...
    hardwareTransactionErrorResponse('wootzWalletApproveTransactionError')
  )
})
...
test('Test sign Trezor transaction, approved, device error', () => {
  return expect(
    ...
    hardwareTransactionErrorResponse('wootzWalletSignOnDeviceError')
  )
})

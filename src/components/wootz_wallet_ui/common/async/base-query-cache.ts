...
export class BaseQueryCache {
  ...
  private _allAccountsInfo?: WootzWallet.AllAccountsInfo
...
//   public rewardsInfo: WootzRewardsInfo | undefined = undefined
  ...
  getNetworksRegistry = async () => {
    if (!this._networksRegistry) {
      ...
      const filteredSupportedCoinTypes = SupportedCoinTypes.filter((coin) => {
        // FIL and SOL networks, unless enabled by wootz://flags
        return (
      ...
      const networkLists: WootzWallet.NetworkInfo[][] = await mapLimit(
        filteredSupportedCoinTypes,
        10,
        async (coin: WootzWallet.CoinType) => {
  ...
  getBalanceScannerSupportedChains = async () => {
    if (!this.balanceScannerSupportedChains) {
      const { wootzWalletService } = getAPIProxy()
      const { chainIds } =
        await wootzWalletService.getBalanceScannerSupportedChains()
  ...
  getIpfsGatewayTranslatedNftUrl = async (urlArg: string) => {
    ...
    if (!this._nftImageIpfsGateWayUrlRegistry[trimmedURL]) {
      const { wootzWalletIpfsService } = getAPIProxy()

      const { translatedUrl } = isIpfs(trimmedURL)
        ? await wootzWalletIpfsService.translateToGatewayURL(trimmedURL || '')
   ...
   * long identifier */
  getTokenLogo = async (token: WootzWallet.BlockchainToken) => {
  ...
  getNftMetadata = async (tokenArg: GetBlockchainTokenIdArg) => {
    ...
    if (
      tokenArg.coin !== WootzWallet.CoinType.ETH &&
      tokenArg.coin !== WootzWallet.CoinType.SOL
    ) {
      throw new Error(
    ...
    if (!this._nftMetadataRegistry[tokenId]) {
      ...
      const metadata: WootzWallet.NftMetadata = result.metadatas[0]
  ...
  getSpamNftsForAccountId = async (accountId: WootzWallet.AccountId) => {
    if (!this.spamNftsForAccountRegistry[accountId.uniqueKey]) {
      ...
      do {
        const {
          ...
          tokens: WootzWallet.BlockchainToken[]
          cursor: string | null
        } = await wootzWalletService.getSimpleHashSpamNFTs(
          address,
          chainIds,
          coin,
          currentCursor
...
async function fetchAssetsForNetwork({
  ...
  const { blockchainRegistry, wootzWalletService } = getAPIProxy()
  ...
  const { tokens } =
    listType === 'known'
      ...
      : await wootzWalletService.getUserAssets(network.chainId, network.coin)

  // Adds a logo and chainId to each token object
  const tokenList: WootzWallet.BlockchainToken[] = await mapLimit(
    tokens,
    10,
...
export async function makeTokensRegistry({
  ...
  const idsByCoinType: Record<WootzWallet.CoinType, string[]> = {}
  ...
  const visibleTokenIdsByCoinType: Record<WootzWallet.CoinType, string[]> = {}
  const hiddenTokenIdsByCoinType: Record<WootzWallet.CoinType, string[]> = {}
  ...
  const fungibleIdsByCoinType: Record<WootzWallet.CoinType, string[]> = {}
  ...
  const nonFungibleIdsByCoinType: Record<WootzWallet.CoinType, string[]> = {}
  ...
  const userTokenListsForNetworks = await mapLimit(
    ...
    10,
    async ([networkId, network]: [string, WootzWallet.NetworkInfo]) => {
      if (!network) {
        return []
      }

      const fullTokensListForNetwork: WootzWallet.BlockchainToken[] =

...
// types
import { WootzWallet, CommonNftMetadata } from '../../../constants/types'
...
export class MockedWalletApiProxy {
  ...
  selectedAccountId: WootzWallet.AccountId = mockAccount.accountId
  ...
  selectedNetwork: WootzWallet.NetworkInfo = mockEthMainnet

  chainIdsForCoins: Record<WootzWallet.CoinType, string> = {
    [WootzWallet.CoinType.ETH]: WootzWallet.MAINNET_CHAIN_ID,
    [WootzWallet.CoinType.SOL]: WootzWallet.SOLANA_MAINNET,
    [WootzWallet.CoinType.FIL]: WootzWallet.FILECOIN_MAINNET
  }

  chainsForCoins: Record<WootzWallet.CoinType, WootzWallet.NetworkInfo> = {
    [WootzWallet.CoinType.ETH]: mockEthMainnet,
    [WootzWallet.CoinType.SOL]: mockSolanaMainnetNetwork,
    [WootzWallet.CoinType.FIL]: mockFilecoinMainnetNetwork
  }

  networks: WootzWallet.NetworkInfo[] = mockNetworks
  ...
  userAssets: WootzWallet.BlockchainToken[] = mockAccountAssetOptions

  evmSimulationResponse: WootzWallet.EVMSimulationResponse | null = null

  svmSimulationResponse: WootzWallet.SolanaSimulationResponse | null = null

  txSimulationOptInStatus: WootzWallet.BlowfishOptInStatus =
    WootzWallet.BlowfishOptInStatus.kAllowed
  ...
  nativeBalanceRegistry: NativeAssetBalanceRegistry = {
    [mockAccount.address]: {
      [WootzWallet.MAINNET_CHAIN_ID]: '0' // 0 ETH
    }
  }
  ...
  transactionInfos: WootzWallet.TransactionInfo[] = [
    ...
    mockFilSendTransaction as WootzWallet.TransactionInfo,
    deserializeTransaction(mockedErc20ApprovalTransaction)
  ]

  // name service lookups
  requireOffchainConsent: number = WootzWallet.ResolveMethod.kAsk

  private pendingAddChainRequests = [mockAddChainRequest]
  private pendingSwitchChainRequests: WootzWallet.SwitchChainRequest[] = [
    mockSwitchChainRequest
  ]

  private pendingDecryptRequests: WootzWallet.DecryptRequest[] = [
    mockDecryptRequest
  ]
  ...
  private signTransactionRequests: WootzWallet.SignTransactionRequest[] = []

  private signAllTransactionsRequests =
    [] as WootzWallet.SignAllTransactionsRequest[]
  ...
  assetsRatioService: Partial<
    InstanceType<typeof WootzWallet.AssetRatioServiceInterface>
  > = {
    getBuyUrlV1: async (
      provider,
      chainId,
  ...
  blockchainRegistry: Partial<
    InstanceType<typeof WootzWallet.BlockchainRegistryInterface>
  > = {
    ...
    getTopDapps: async (chainId, coin) => {
      switch (chainId) {
        ...
        case WootzWallet.MAINNET_CHAIN_ID: {
        ...
        case WootzWallet.SOLANA_MAINNET: {
  ...
  wootzWalletService: Partial<
    InstanceType<typeof WootzWallet.WootzWalletServiceInterface>
  > = {
    getUserAssets: async (chainId: string, coin: WootzWallet.CoinType) => {
      return {
  ...
  swapService: Partial<InstanceType<typeof WootzWallet.SwapServiceInterface>> =
    {
      getTransaction: async (
        params: WootzWallet.SwapTransactionParamsUnion
      ): Promise<{
        response: WootzWallet.SwapTransactionUnion | null
        error: WootzWallet.SwapErrorUnion | null
        errorString: string
      }> => {
      ...
      getQuote: async (
        params: WootzWallet.SwapQuoteParams
      ): Promise<{
        response: WootzWallet.SwapQuoteUnion | null
        fees: WootzWallet.SwapFees | null
        error: WootzWallet.SwapErrorUnion | null
        errorString: string
      }> => ({
        response: {
        ...
        fees: {
          ...
          discountCode: WootzWallet.SwapDiscountCode.kNone
        },
        error: null,
        errorString: ''
      })
  ...
  keyringService: Partial<
    InstanceType<typeof WootzWallet.KeyringServiceInterface>
  > = {
    getAllAccounts: async (): Promise<{
      allAccounts: WootzWallet.AllAccountsInfo
    }> => {
      ...
      const allAccounts: WootzWallet.AllAccountsInfo = {
  ...
  ethTxManagerProxy: Partial<
    InstanceType<typeof WootzWallet.EthTxManagerProxyInterface>
  > = {
    getGasEstimation1559: async () => {
      return {
        estimation: {
          ...
          fastMaxFeePerGas: '0',
          baseFeePerGas: '0'
        } as WootzWallet.GasEstimation1559 | null
      }
    }
  }

  wootzWalletP3A: InstanceType<typeof WootzWallet.WootzWalletP3AInterface> = {
  ...
  jsonRpcService: Partial<
    InstanceType<typeof WootzWallet.JsonRpcServiceInterface>
  > = {
    ...
    notifySwitchChainRequestProcessed: (requestId, approved) => {
      ...
      if (request) {
        ...
        this.wootzWalletService.setNetworkForSelectedAccountOnActiveOrigin?.(
          request.chainId
        )
      }
    },
    ...
    getERC20TokenAllowance: async (
      ...
      return {
        ...
        error: WootzWallet.ProviderError.kSuccess,
        errorMessage: ''
      }
    },
    // NFT Metadata
    ...
    getNftMetadatas: async (coin, nftIdentifiers) => {
      const metadatas: WootzWallet.NftMetadata[] = nftIdentifiers.map((id) => {
    ...
    ensGetEthAddr: async (domain) => {
      return {
        ...
        requireOffchainConsent:
          this.requireOffchainConsent !== WootzWallet.ResolveMethod.kEnabled
      }
    },
    ...
    getEthTokenInfo: async (contractAddress, chainId) => {
      ...
      if (contractAddress === '0xInvalidToken') {
        return {
          token: null,
          error: WootzWallet.ProviderError.kResourceNotFound,
      ...
      return {
        token: {
          ...
          isErc721: false,
          isErc1155: false,
          splTokenProgram: WootzWallet.SPLTokenProgram.kUnsupported,
          isNft: false,
          isCompressed: false,
          tokenId: '',
          logo: '',
  ...
  solanaTxManagerProxy: Partial<
    InstanceType<typeof WootzWallet.SolanaTxManagerProxyInterface>
  > = {
  ...
  walletHandler: Partial<
    InstanceType<typeof WootzWallet.WalletHandlerInterface>
  > = {
    getWalletInfo: async (): Promise<{
      walletInfo: WootzWallet.WalletInfo
    }> => {
      return {
        walletInfo: {
  ...
  txService: Partial<InstanceType<typeof WootzWallet.TxServiceInterface>> = {
  ...
  simulationService: Partial<
    InstanceType<typeof WootzWallet.SimulationServiceInterface>
  > = {
    ...
    scanEVMTransaction: async (txInfo, language) => {
      return {
        ...
        errorString: '',
        response: this
          .evmSimulationResponse as WootzWallet.EVMSimulationResponse
      }
    },
    scanSolanaTransaction: async (request, language) => {
      return {
        ...
        errorString: '',
        response: this
          .svmSimulationResponse as WootzWallet.SolanaSimulationResponse
      }
    }
  }

  wootzWalletIpfsService: Partial<
    InstanceType<typeof WootzWallet.IpfsServiceInterface>
  > = {
...
function createDappParserForRange(
  range: string
): (value: DappListResult) => WootzWallet.Dapp {
  return (d) => ({

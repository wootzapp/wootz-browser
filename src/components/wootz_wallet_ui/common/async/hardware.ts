...
export async function signTrezorTransaction(
  ...
  if (!nonce || !nonce.nonce) {
    return {
      success: false,
      error: getLocale('wootzWalletApproveTransactionError')
    }
  }
  ...
  if (!signed || !signed.success || !signed.payload) {
    const error = (
      signed.error ? signed.error : getLocale('wootzWalletSignOnDeviceError')
    ) as string
  ...
  if (!result.status) {
    return {
      success: false,
      error: getLocale('wootzWalletProcessTransactionError')
    }
  }
...
export async function signLedgerEthereumTransaction(
  ...
  if (!data || !data.message || !data.message.messageStr) {
    return {
      success: false,
      error: getLocale('wootzWalletNoMessageToSignError')
    }
  }
  ...
  if (!signed || !signed.success || !signed.payload) {
    const error = signed?.error ?? getLocale('wootzWalletSignOnDeviceError')
...
export async function signMessageWithHardwareKeyring(
  ...
  path: string,
  messageData: Omit<WootzWallet.SignMessageRequest, 'originInfo'>
): Promise<SignHardwareOperationResult> {
  ...
  if (deviceKeyring instanceof EthereumLedgerBridgeKeyring) {
    if (signTypedData) {
      if (!signTypedData.domainHash || !signTypedData.primaryHash) {
        return {
          success: false,
          error: getLocale('wootzWalletUnknownInternalError')
        }
      }
    ...
    return {
      success: false,
      error: getLocale('wootzWalletHardwareOperationUnsupportedError')
    }
  }
  return { success: false, error: getLocale('wootzWalletUnknownKeyringError') }
}

export async function signRawTransactionWithHardwareKeyring(
  vendor: HardwareVendor,
  path: string,
  message: WootzWallet.ByteArrayStringUnion,
  coin: WootzWallet.CoinType,
  onAuthorized?: () => void
): Promise<SignHardwareOperationResult> {
...
export const onConnectHardwareWallet = async (
...
): Promise<WootzWallet.HardwareWalletAccount[]> => {
  ...
  if ((isLedger || isTrezor) && opts.scheme) {
    ...
    if (result.payload) {
      ...
      const addressesEncoded = await wootzWalletService.base58Encode(

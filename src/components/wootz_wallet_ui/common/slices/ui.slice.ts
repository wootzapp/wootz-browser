...
import { WootzWallet, UIState } from '../../constants/types'
        ...
        (state, { payload }) => {
          ...
          if (!state.selectedPendingTransactionId) {
            const firstPendingTx = payload.find(
              (tx) => tx.txStatus === WootzWallet.TransactionStatus.Unapproved
            )
            if (firstPendingTx) {
      ...
      builder.addMatcher(
        ...
        (state, { payload }) => {
          ...
          if (
            ...
            payload.status === WootzWallet.TransactionStatus.Unapproved
          ) {
            state.selectedPendingTransactionId = payload.txId
          }
        }

...
interface ProcessSignMessageRequestArgs {
  approved: boolean
  id: number
  signature?: WootzWallet.ByteArrayStringUnion | undefined
...
export const signingEndpoints = ({
  ...
  return {
    getPendingSignMessageRequests: query<
      WootzWallet.SignMessageRequest[],
      void
    >({
      queryFn: async (_arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { requests } =
            await api.wootzWalletService.getPendingSignMessageRequests()
    ...
    getPendingSignMessageErrors: query<WootzWallet.SignMessageError[], void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)

          const { errors } =
            await api.wootzWalletService.getPendingSignMessageErrors()
    ...
    >({
      queryFn: async (errorIdArg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          api.wootzWalletService.notifySignMessageErrorProcessed(errorIdArg)
    ...
    signMessageHardware: mutation<
      ...
      {
        request: WootzWallet.SignMessageRequest
      ...
      queryFn: async (arg, { endpoint, ...store }, extraOptions, baseQuery) => {
        try {
          ...
          if (!isHardwareAccount(arg.account.accountId)) {
            api.wootzWalletService.notifySignMessageRequestProcessed(
              false,
              arg.request.id,
              null,
              getLocale('wootzWalletHardwareAccountNotFound')
            )
          ...
          if (!signed.success && signed.code) {
            ...
            const deviceError =
              info.vendor === WootzWallet.TREZOR_HARDWARE_VENDOR

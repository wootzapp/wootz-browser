...
export const networkEndpoints = ({
  ...
  return {
    ...
    getAllKnownNetworks: query<WootzWallet.NetworkInfo[], void>({
    ...
    getSwapSupportedNetworks: query<WootzWallet.NetworkInfo[], void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const {
            data: { swapService },
    ...
    getSelectedChain: query<WootzWallet.NetworkInfo | null, void>({
      queryFn: async (_arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { network } = await api.wootzWalletService //
    ...
    getPendingAddChainRequest: query<WootzWallet.AddChainRequest | null, void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
    ...
    getPendingSwitchChainRequest: query<
      WootzWallet.SwitchChainRequest | null,
      void
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
    ...
    hideNetworks: mutation<
      boolean,
      Array<Pick<WootzWallet.NetworkInfo, 'chainId' | 'coin'>>
    >({
      queryFn: async (
        chains,
        { endpoint, dispatch },
        ...
        try {
          ...
          await mapLimit(
            ...
            10,
            async (
              chain: Pick<WootzWallet.NetworkInfo, 'chainId' | 'coin'>
            ) => {
    ...
    setNetwork: mutation<
      ...
      Pick<WootzWallet.NetworkInfo, 'chainId' | 'coin'>
    >({
      queryFn: async (
        { chainId, coin },
        { endpoint },
        ...
        try {
          ...
          const { accountId: selectedAccountId } =
            await wootzWalletService.ensureSelectedAccountForChain(
              coin,
              chainId
            )

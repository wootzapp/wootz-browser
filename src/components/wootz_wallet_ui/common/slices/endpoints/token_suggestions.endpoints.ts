...
export const tokenSuggestionsEndpoints = ({
  ...
  return {
    getPendingTokenSuggestionRequests: query<
      WootzWallet.AddSuggestTokenRequest[],
      void
    >({
      queryFn: async (_arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          ...
          const { requests } =
            await api.wootzWalletService.getPendingAddSuggestTokenRequests()
    ...
    >({
      queryFn: async (arg, { endpoint }, _extraOptions, baseQuery) => {
        ...
        try {
          apiProxy.wootzWalletService.notifyAddSuggestTokenRequestsProcessed(
            arg.approved,
            [arg.contractAddress]
          )

  ...
  return {
    getNftDiscoveryEnabledStatus: query<boolean, void>({
      queryFn: async (_arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          ...
          const result = await wootzWalletService.getNftDiscoveryEnabled()
          return {
            data: result.enabled
          }
        } catch (error) {
    ...
    >({
      queryFn: async (arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          const { wootzWalletService } = baseQuery(undefined).data
          await wootzWalletService.setNftDiscoveryEnabled(arg)
      ...
      async onCacheEntryAdded(
        ...
        try {
          ...
          try {
            ...
            const uniqueTokens: WootzWallet.BlockchainToken[] = arg.filter(
              (t) => {
            ...
            for (const token of uniqueTokens) {
              ...
              if (token.coin === WootzWallet.CoinType.ETH) {
                const {
                  token: tokenInfo,
                  error,
                  errorMessage
    ...
    getSimpleHashSpamNfts: query<
      WootzWallet.BlockchainToken[],
      void | undefined | { accounts: WootzWallet.AccountInfo[] }
    >({
      queryFn: async (arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          const { cache } = baseQuery(undefined)

...
export const sitePermissionEndpoints = ({
  ...
  return {
    getActiveOriginConnectedAccountIds: query<WootzWallet.AccountId[], void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { accountsWithPermission } =
            await wootzWalletService.hasPermission(
    ...
    getIsPrivateWindow: query<boolean, void>({
      queryFn: async (_, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { isPrivateWindow } = await wootzWalletService.isPrivateWindow()
    ...
    connectToSite: mutation<
      true,
      {
        ...
        duration: WootzWallet.PermissionLifetimeOption
      }
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
    ...
    requestSitePermission: mutation<true, WootzWallet.AccountId>({
    ...
    removeSitePermission: mutation<true, WootzWallet.AccountId>({
      queryFn: async (accountId, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { wootzWalletService } = api

          await wootzWalletService.resetPermission(accountId)

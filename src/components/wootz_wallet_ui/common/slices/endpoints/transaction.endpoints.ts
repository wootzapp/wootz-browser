...
interface ProcessSignTransactionRequestPayload {
  approved: boolean
  id: number
  signature?: WootzWallet.ByteArrayStringUnion | null
  error?: string | null
}
  ...
  return {
    ...
    getTransactions: query<
      SerializableTransactionInfo[],
      {
         ...
         */
        coinType: WootzWallet.CoinType | null
        /**
         * will fetch for all coin-type addresses if null
         */
        accountId: WootzWallet.AccountId | null
        /**
      ...
      queryFn: async (
        ...
        try {
          ...
          const nonRejectedTransactionInfos = txInfos
            // hide rejected txs
            .filter(
              (tx) => tx.txStatus !== WootzWallet.TransactionStatus.Rejected
            )
    ...
    sendSolanaSerializedTransaction: mutation<
      boolean, // success,
      {
        ...
        txType: WootzWallet.TransactionType
        sendOptions?: WootzWallet.SolanaSendTransactionOptions
      }
    >({
      queryFn: async (payload, { endpoint }, extraOptions, baseQuery) => {
        try {
    ...
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { requests } =
            await api.wootzWalletService.getPendingSignTransactionRequests()
    ...
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const { requests } =
            await api.wootzWalletService.getPendingSignAllTransactionsRequests()
    ...
    processSignTransactionRequestHardware: mutation<
      ...
      {
        request: WootzWallet.SignTransactionRequest
    ...
    >({
      queryFn: async (arg, { endpoint, ...store }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { wootzWalletService, panelHandler } = api

          if (!isHardwareAccount(arg.account.accountId)) {
            const errorString = getLocale('wootzWalletHardwareAccountNotFound')

            wootzWalletService.notifySignTransactionRequestProcessed(
              false,
              arg.request.id,
              null,
              errorString
          ...
          processSignTransactionRequest(
            ...
            signed.success
              ? {
                  ...
                  id: arg.request.id,
                  signature:
                    arg.account.accountId.coin === WootzWallet.CoinType.SOL
    ...
    processSignAllTransactionsRequestHardware: mutation<
      { success: boolean; errorCode?: string | number },
      {
        request: WootzWallet.SignAllTransactionsRequest
        account: WootzWallet.AccountInfo
      }
    >({
      queryFn: async (arg, { endpoint, ...store }, extraOptions, baseQuery) => {
        try {
          ...
          if (!isHardwareAccount(arg.account.accountId)) {
            ...
            wootzWalletService.notifySignAllTransactionsRequestProcessed(
              false,
              arg.request.id,
              null,
              errorString
    ...
    >({
      queryFn: async (payload, { dispatch }, extraOptions, baseQuery) => {
        try {
          ...
          const btcTxData: WootzWallet.BtcTxData = {
            to: payload.to,
    ...
    >({
      queryFn: async (payload, { dispatch }, extraOptions, baseQuery) => {
        try {
          ...
          const zecTxData: WootzWallet.ZecTxData = {
            to: payload.to,
            amount: BigInt(payload.value),
            fee: BigInt(0),
            inputs: [],
    ...
    >({
      queryFn: async (payload, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const filTxData: WootzWallet.FilTxData = {
            nonce: payload.nonce || '',
      ...
      invalidatesTags: (res, err, arg) =>
        err
          ...
          : [
              ...
              ...(arg.txStatus === WootzWallet.TransactionStatus.Confirmed
                ? ([
    ...
    approveTransaction: mutation<
      {
        success: boolean
        errorUnion: WootzWallet.ProviderErrorUnion
        errorMessage: string
      },
    ...
    >({
      queryFn: async (txInfo, store, extraOptions, baseQuery) => {
        try {
          ...
          const hardwareAccount: WootzWallet.HardwareInfo =
          ...
          if (hardwareAccount.vendor === WootzWallet.LEDGER_HARDWARE_VENDOR) {
            ...
            hardwareAccount.vendor === WootzWallet.TREZOR_HARDWARE_VENDOR
          ) {
    ...
    rejectTransactions: mutation<
      { success: boolean },
      Array<
        Pick<WootzWallet.TransactionInfo, 'id' | 'chainId'> & {
          coinType: WootzWallet.CoinType
        }
      >
    >({
      queryFn: async (txs, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          await mapLimit(
            ...
            10,
            async (
              tx: Pick<WootzWallet.TransactionInfo, 'id' | 'chainId'> & {
                coinType: WootzWallet.CoinType
              }
            ) => {
      ...
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const priorityFee =
            ...
            BigInt(WootzWallet.MICRO_LAMPORTS_PER_LAMPORT)
...
async function sendEvmTransaction({
  ...
  txService: WootzWallet.TxServiceRemote
}): Promise<{ data: { success: boolean } }> {
  const params: WootzWallet.NewEvmTransactionParams = {
...
async function processSignTransactionRequest(
  ...
  panelHandler: WootzWallet.PanelHandlerRemote | undefined
) {
  wootzWalletService.notifySignTransactionRequestProcessed(
    arg.approved,
    arg.id,

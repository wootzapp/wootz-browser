...
export const tokenEndpoints = ({
  ...
  return {
    ...
    addUserToken: mutation<{ id: EntityId }, WootzWallet.BlockchainToken>({
      queryFn: async (
        tokenArg,
        { dispatch, endpoint },
        extraOptions,
    ...
    >({
      queryFn: async (tokenId, { endpoint }, extraOptions, baseQuery) => {
        ...
        try {
          ...
          if (token) {
            const deleteResult = await wootzWalletService.removeUserAsset(token)
            if (!deleteResult.success) {
              throw new Error(
                'wootzWalletService.removeUserAsset was unsuccessful'
              )
            }
          }
    ...
    updateUserToken: mutation<
      ...
      string,
      {
        existingToken: WootzWallet.BlockchainToken
        updatedToken: WootzWallet.BlockchainToken
      }
    >({
      queryFn: async (
        ...
        try {
          ...
          // update core user assets list
          const deleteResult = await wootzWalletService.removeUserAsset(
            existingToken
          )
    ...
    >({
      queryFn: async (
        ...
        try {
          ...
          const { success } = await wootzWalletService.setUserAssetVisible(
            token,
            isVisible
          )
    ...
    getTokenInfo: query<
      WootzWallet.BlockchainToken | null,
      Pick<WootzWallet.BlockchainToken, 'chainId' | 'coin' | 'contractAddress'>
    >({
      queryFn: async (
        ...
        try {
          ...
          if (error !== WootzWallet.ProviderError.kSuccess) {
    ...
    discoverAssets: mutation<true, void>({
      queryFn: async (
        ...
        try {
          const { data: api } = baseQuery(undefined)
          api.wootzWalletService.discoverAssetsOnAllSupportedChains(true)
          return {
            data: true
          }
        } catch (error) {
    ...
    >({
      queryFn: async (arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          ...
          const { success } = await wootzWalletService.setAssetSpamStatus(
            arg.token,
            arg.isSpam
          )
...
async function addUserToken({
  ...
  tokenArg: WootzWallet.BlockchainToken
}) {
  ...
  return await wootzWalletService.addUserAsset({
    ...tokenArg,

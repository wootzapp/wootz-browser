...
export const encryptionEndpoints = ({
  ...
  return {
    getPendingDecryptRequest: query<WootzWallet.DecryptRequest | null, void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)

          const { requests } =
            await api.wootzWalletService.getPendingDecryptRequests()
    ...
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          api.wootzWalletService.notifyDecryptRequestProcessed(
            arg.requestId,
            arg.approved
          )
    ...
    getPendingGetEncryptionPublicKeyRequest: query<
      WootzWallet.GetEncryptionPublicKeyRequest | null,
      void
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
    ...
    >({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          wootzWalletService.notifyGetPublicKeyRequestProcessed(
            arg.requestId,
            arg.approved
          )

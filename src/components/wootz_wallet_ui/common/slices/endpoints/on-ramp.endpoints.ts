...
// Types
import { WootzWallet, SupportedOnRampNetworks } from '../../../constants/types'
...
export const onRampEndpoints = ({ query }: WalletApiEndpointBuilderParams) => {
  return {
    getOnRampAssets: query<
      {
        rampAssetOptions: WootzWallet.BlockchainToken[]
        sardineAssetOptions: WootzWallet.BlockchainToken[]
        transakAssetOptions: WootzWallet.BlockchainToken[]
        stripeAssetOptions: WootzWallet.BlockchainToken[]
        coinbaseAssetOptions: WootzWallet.BlockchainToken[]
        allAssetOptions: WootzWallet.BlockchainToken[]
      },
      void
    >({
      queryFn: async (_arg, _store, _extraOptions, baseQuery) => {
        try {
          ...
          const updateLogo = async (token: WootzWallet.BlockchainToken) => {
            const tokenLogo = await cache.getTokenLogo(token)
            return addLogoToToken(token, tokenLogo)
          }

          // add token logos
          const rampAssetOptions: WootzWallet.BlockchainToken[] =
            await mapLimit(
          ...
          const sardineAssetOptions: WootzWallet.BlockchainToken[] =
            await mapLimit(
          ...
          const transakAssetOptions: WootzWallet.BlockchainToken[] =
            await mapLimit(
          ...
          const stripeAssetOptions: WootzWallet.BlockchainToken[] =
            await mapLimit(
          ...
          const coinbaseAssetOptions: WootzWallet.BlockchainToken[] =
            await mapLimit(
    ...
    getOnRampFiatCurrencies: query<WootzWallet.OnRampCurrency[], void>({
    ...
    getBuyUrl: query<
      string,
      {
        onRampProvider: WootzWallet.OnRampProvider
        chainId: string
        address: string
        assetSymbol: string
        amount: string
...
// internals
function getRampProviderName(onRampProvider: WootzWallet.OnRampProvider) {
  return Object.keys(WootzWallet.OnRampProvider)
    .find((key) => WootzWallet.OnRampProvider[key] === onRampProvider)
    ?.substring(1)
}

...
export const dappRadarEndpoints = ({
  ...
  return {
    getTopDapps: query<WootzWallet.Dapp[], void>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          const topDapps: WootzWallet.Dapp[] = await flatMapLimit(
            networks,
            10,
            async (net: WootzWallet.NetworkInfo) => {
          ...
          const uniqueTopDapps: WootzWallet.Dapp[] = []
          ...
          const parsedUniqueTopDapps: WootzWallet.Dapp[] = uniqueTopDapps.map(
            (dapp) => {
              return {
                ...dapp,
                chains: dapp.chains

...
export const useAccountQuery = (
  accountId: WootzWallet.AccountId | undefined | typeof skipToken,
  opts?: { skip?: boolean }
) => {
...
export const useGetCombinedTokensListQuery = (
  ...
  const combinedQuery = React.useMemo(() => {
    if (isLoadingUserTokens || isLoadingKnownTokens) {
      return {
        isLoading: true,
        data: [] as WootzWallet.BlockchainToken[]
      }
    }
...
export const useTransactionsNetworkQuery = <
  T extends
    | Pick<
        SerializableTransactionInfo | WootzWallet.TransactionInfo,
        'chainId' | 'txDataUnion'
      >
    | undefined
    | typeof skipToken
...
export const useReceiveAddressQuery = (
  accountId: WootzWallet.AccountId | undefined
) => {
  // state

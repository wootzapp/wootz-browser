...
export default class TrezorBridgeKeyring implements TrezorKeyring {
  ...
  keyringId = (): WootzWallet.KeyringId => {
  ...
  unlock = async (): Promise<HardwareOperationResult> => {
    ...
    if (!data.payload.success) {
      ...
      const error =
        response.payload?.error ?? getLocale('wootzWalletUnlockError')
  ...
  signEip712Message = async (
    ...
    if (!response.success) {
      ...
      if (unsuccess.code && unsuccess.code === 'Method_InvalidParameter') {
        return {
          success: false,
          error: getLocale('wootzWalletTrezorSignTypedDataError')
        }
      }
      return { success: false, error: unsuccess.error, code: unsuccess.code }
    }
  ...
  private readonly createErrorFromCode = (
    ...
    const deviceName = getLocale('wootzWalletConnectHardwareTrezor')

    switch (code) {
      case TrezorErrorsCodes.BridgeNotReady:
        return {
          success: false,
          error: getLocale('wootzWalletBridgeNotReady').replace(
            '$1',
            deviceName
          ),
          code: code
      ...
      case TrezorErrorsCodes.CommandInProgress:
        return {
          success: false,
          error: getLocale('wootzWalletBridgeCommandInProgress').replace(
            '$1',
            deviceName
          ),
          code: code
